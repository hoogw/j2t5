...
var NW_url_returngeojson
var NE_url_returngeojson
var SE_url_returngeojson
var SW_url_returngeojson
var url_4x = {
                NW: '',
                NE: '',
                SE: '',
...
function ajax_GeoJSON() {
    ...
    NW_envelope = encodeURI('{"spatialReference":{"wkid":4326}, "xmin" : '+ SWlong +', "ymin" : '+ _center_lat + ', "xmax" : '+ _center_long +', "ymax" : '+ NElat + '}')
    NE_envelope = encodeURI('{"spatialReference":{"wkid":4326}, "xmin" : '+ _center_long +', "ymin" : '+ _center_lat + ', "xmax" : '+ NElong +', "ymax" : '+ NElat + '}')
    SE_envelope = encodeURI('{"spatialReference":{"wkid":4326}, "xmin" : '+ _center_long +', "ymin" : '+ SWlat + ', "xmax" : '+ NElong +', "ymax" : '+ _center_lat + '}')
    SW_envelope = encodeURI('{"spatialReference":{"wkid":4326}, "xmin" : '+ SWlong +', "ymin" : '+ SWlat + ', "xmax" : '+ _center_long +', "ymax" : '+ _center_lat + '}')


      //_url_returngeojson += '&geometry='+ _envelope;
      NW_url_returngeojson = _url_returngeojson + '&geometry=' + NW_envelope;
      NE_url_returngeojson = _url_returngeojson + '&geometry=' + NE_envelope;
      SE_url_returngeojson = _url_returngeojson + '&geometry=' + SE_envelope;
      SW_url_returngeojson = _url_returngeojson + '&geometry=' + SW_envelope;

      url_4x.NW = NW_url_returngeojson
      url_4x.NE = NE_url_returngeojson
      url_4x.SE = SE_url_returngeojson
      url_4x.SW = SW_url_returngeojson

      //ajax_GeoJSON(_url_returncountonly, _url_returngeojson);
      ajax_GeoJSON_countOnly(_url_returncountonly);
      ajax_GeoJSON_4x(url_4x);
...
function ajax_GeoJSON_countOnly(_apiURI_returncountonly) {
  ...
  var  ___XMLHttpRequest_NW;
  var  ___XMLHttpRequest_NE;
  var  ___XMLHttpRequest_SE;
  var  ___XMLHttpRequest_SW;
  


  
function ajax_GeoJSON_4x(_apiURI_4x) {
    //  ***** abort previously  ajax call   ***** 

    // NW
    if (___XMLHttpRequest_NW){
     if (typeof ___XMLHttpRequest_NW.abort !== "undefined"){
       console.log('___XMLHttpRequest_NW - xhr ', ___XMLHttpRequest_NW)
       // abort will cause jsonp call back error(normal, no need fix) :  Uncaught ReferenceError jqueryxxxxx_xxxxx  is not defined
       ___XMLHttpRequest_NW.abort() 
       }
     }

     // NE
     if (___XMLHttpRequest_NE){
       if (typeof ___XMLHttpRequest_NE.abort !== "undefined"){
         console.log('___XMLHttpRequest_NE - xhr ', ___XMLHttpRequest_NE)
         // abort will cause jsonp call back error(normal, no need fix) :  Uncaught ReferenceError jqueryxxxxx_xxxxx  is not defined
         ___XMLHttpRequest_NE.abort() 
         }
       }


     // SE
     if (___XMLHttpRequest_SE){
       if (typeof ___XMLHttpRequest_SE.abort !== "undefined"){
         console.log('___XMLHttpRequest_SE - xhr ', ___XMLHttpRequest_SE)
         // abort will cause jsonp call back error(normal, no need fix) :  Uncaught ReferenceError jqueryxxxxx_xxxxx  is not defined
         ___XMLHttpRequest_SE.abort() 
         }
       }


       // SW
     if (___XMLHttpRequest_SW){
       if (typeof ___XMLHttpRequest_SW.abort !== "undefined"){
         console.log('___XMLHttpRequest_SW - xhr ', ___XMLHttpRequest_SW)
         // abort will cause jsonp call back error(normal, no need fix) :  Uncaught ReferenceError jqueryxxxxx_xxxxx  is not defined
         ___XMLHttpRequest_SW.abort() 
         }
       }

     //  ******* end ***** abort previously  ajax call   *****   
                       
     console.log('ajax url-- with-geometry ---> : ', _apiURI_4x);
       ...
       $.ajax({
         ...
         beforeSend:  function( jqXHR_NW, settings ){
                                       ___XMLHttpRequest_NW = jqXHR_NW
                      },
         error: function (jqXHR_NW, textStatus, errorThrown) {
                 var _error_status = textStatus + ' : ' + errorThrown;         
                 console.log('jqXHR_NW  : ', _error_status);
                }, 
         success: function (data) {
                     // jsonp, usually return object.   cors, proxy, can return both string and object, must handle difference  
                     if (typeof data === 'object') {
       ...
       $.ajax({
         ...
         beforeSend:  function( jqXHR_NE, settings ){
                                       ___XMLHttpRequest_NE = jqXHR_NE
                      },
         error: function (jqXHR_NE, textStatus, errorThrown) {
                 var _error_status = textStatus + ' : ' + errorThrown;         
                 console.log('jqXHR_NE  : ', _error_status);
                }, 
         success: function (data) {
                     // jsonp, usually return object.   cors, proxy, can return both string and object, must handle difference  
                     if (typeof data === 'object') {
       ...
       $.ajax({
         ...
         beforeSend:  function( jqXHR_SE, settings ){
                                       ___XMLHttpRequest_SE = jqXHR_SE
                      },
         error: function (jqXHR_SE, textStatus, errorThrown) {
                 var _error_status = textStatus + ' : ' + errorThrown;         
                 console.log('jqXHR_SE  : ', _error_status);
                }, 
         success: function (data) {
                     // jsonp, usually return object.   cors, proxy, can return both string and object, must handle difference  
                     if (typeof data === 'object') {
       ...
       $.ajax({
         ...
         beforeSend:  function( jqXHR_SW, settings ){
                                       ___XMLHttpRequest_SW = jqXHR_SW
                      },
         error: function (jqXHR_SW, textStatus, errorThrown) {
                 var _error_status = textStatus + ' : ' + errorThrown;         
                 console.log('jqXHR_SW  : ', _error_status);
                }, 
         success: function (data) {
                     // jsonp, usually return object.   cors, proxy, can return both string and object, must handle difference  
                     if (typeof data === 'object') {
