              ...
              //self-run
              (async function init_map_component_event(){ //  --- nearmap  --- 
                ...
                arcgisMap.addEventListener("arcgisViewReadyChange", (event) => {
                  ...
                  // first time zoom to layer must wait until view is ready, otherwise, may not zoom to.
                  // move to after create feature layer
...
// component // view . on ( " pointer-move " , function(event){
  ...
  async function init_nearmap(){

     
        //1st time one time
          urlParams = new URLSearchParams(window.location.search);
          var param_nearmap_api_key = urlParams.get('nearmapkey'); 
          if (param_nearmap_api_key){
            nearmap_api_key = param_nearmap_api_key
          }//if
          $('#nearmap-key-input').val(nearmap_api_key)

          // bug, 1st time nearmap would not init correctly, blank, force to use google by default 
          //esri_basemap_id = 'Google'
          //arcgisMap.basemap = google_hybrid
          //  --- end  ---1st time one time
          


          $('#nearmap-button').on("click", (event) => {
            nearmap_api_key = $('#nearmap-key-input').val()
            esri_basemap_id = 'nearmap'
            ...
            nearmap = new Basemap({
                  baseLayers: [new WebTileLayer({
                    urlTemplate : "https://api.nearmap.com/tiles/v3/Vert/{level}/{col}/{row}.img?apikey=" + nearmap_api_key, 
                  ...
                  id: "NearMap",
                  title: "NearMap", // icon already has text
                  thumbnailUrl: "../../public/images/nearmap000.png",
                });

            arcgisMap.basemap = nearmap

            // uncheck all other base map radio
            $('input:radio[name="basemap_radio"]').prop('checked', false);
          });
