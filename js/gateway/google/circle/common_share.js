...
/**/
        ...
        var max_circle_radius
        var current_circle_radius
        var max_circle_radius_miles
        var current_circle_radius_miles
        ...
        function manually_adjust_max_circle_radius(){
          $("#circle_radius_range").attr("max", max_circle_radius);
          $("#circle_radius_range").val(current_circle_radius);

          var html_currentValue = '<span font-size:13.6px; style="font-weight:800">' + current_circle_radius + ' </span>meters'
          html_currentValue += '<span font-size:16.6px; style="font-weight:800">(' + current_circle_radius_miles + ' </span>miles)'
          $("#circle_radius_value_label").html(html_currentValue);

          var html_maxValue = '<span style="font-size:10.7px">Max circle radius: <span style="font-size:16.6px; font-weight:800">' + max_circle_radius + ' </span>meters'
          html_maxValue += '<span style="font-size:12.6px; font-weight:800">(' + max_circle_radius_miles + ' </span>miles)'
          html_maxValue += ' otherwise circle expand beyond current map view'
          $('#manual_circle_radius_div').html(html_maxValue);

      }
        

        function get_circle_radius_in_map_bound(){
          ...
          var min_circle_radius
          if (vertical_distance <= horizontal_distance){
            min_circle_radius = vertical_distance
          } else {
            min_circle_radius = horizontal_distance
          }

          console.log("min_circle_radius in meter", min_circle_radius)
          max_circle_radius =  Math.floor(min_circle_radius)
          //current_circle_radius = Math.floor(max_circle_radius / 4)
          current_circle_radius = Math.floor(max_circle_radius / 8)

          max_circle_radius_miles =  (max_circle_radius * 0.000621371).toFixed(2)
          current_circle_radius_miles = (current_circle_radius * 0.000621371).toFixed(2)
        }
        ...
        function drawing_shape(_circleRadiusMeter, _centerLngLatPoint){

          console.log('drawing circle at _circleRadiusMeter, _centerLngLatPoint', _circleRadiusMeter, _centerLngLatPoint)



          google_shape = new google.maps.Circle({
            strokeColor: "#FF0000",
            strokeOpacity: 0.8,
            strokeWeight: 2,
            fillColor: "#FF0000",
            fillOpacity: 0.25,
            map,
            center: { lat: _centerLngLatPoint.lat, lng: _centerLngLatPoint.lng },
            radius:  _circleRadiusMeter,
          });

        
          google_shape_array.push(google_shape)
        ...
        async function select_feature_intersect_with_circle(_circleRadiusMeter, _centerLngLatPoint){

                  console.log('select feature intersect with circle at _circleRadiusMeter, _centerLngLatPoint', _circleRadiusMeter, _centerLngLatPoint)

         
 
     
                  // circle  api,  circlular arc, a property :   https://developers.arcgis.com/rest/services-reference/enterprise/geometry-objects/

                  // endPoint set as mid-north point
                  mid_north = google.maps.geometry.spherical.computeOffset(_centerLngLatPoint, _circleRadiusMeter, 0);
                  mid_north_lat = mid_north.lat();
                  mid_north_lng = mid_north.lng();
                  //console.log('mid_north', mid_north_lng, mid_north_lat)

                  var endPoint_lat = mid_north_lat
                  var endPoint_lng = mid_north_lng

                  var centerPoint_lat = _centerLngLatPoint.lat
                  var centerPoint_lng = _centerLngLatPoint.lng



                  var _circle_un_encode = '{"spatialReference":{"wkid":4326},'


                  // see sample here https://developers.arcgis.com/rest/services-reference/enterprise/geometry-objects/#polygon-with-curves
                  // {
                  //    "hasM": true,
                  //    "curveRings":
                  //    [
                  ...
                  //     ]
                  //  }
                  // never use rings, should use curveRings
                  //_circle_un_encode += '"rings":[['
                  _circle_un_encode += '"curveRings":[['
                  // must add start point, which is mid-north point
                  _circle_un_encode += '['+ endPoint_lng + ',' + endPoint_lat + '],'
                  // add curve, as 'a' property
                  _circle_un_encode += '{"a":[ ['+ endPoint_lng + ',' + endPoint_lat + '], [' + centerPoint_lng +  ',' + centerPoint_lat + '], 0, 1] },';
                  // must add end point, which is same as start point
                  _circle_un_encode += '['+ endPoint_lng + ',' + endPoint_lat + '],'
                  ...
                  var _circle = encodeURI(_circle_un_encode);
                  console.log('_circle --- encoded >>>', _circle)         
        ...
        // only for google manual drawing circle
        function add_ground_overlay_listener_click(){
          ...
          var listener_ground_overlay_click =  groundoverlay.addListener('click', function(click_event) {   
            ...
            drawing_shape(current_circle_radius, click_lat_lng_point)

            select_feature_intersect_with_circle(current_circle_radius, click_lat_lng_point)
                ...
                /**/
                                ...
                                groundoverlay.addListener('mousemove', function (event) {  

                                    // draw guided ring based on mouse pointer lat lng
