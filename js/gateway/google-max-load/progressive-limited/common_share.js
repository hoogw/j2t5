...
var sync_async = 'async-get'


           // ajax.abort - can not use async-await 
           function progressive_getObjectID(_apiURI_returncountonly, _apiURI) {
                                              ...
                                              beforeSend:  function( jqXHR, settings ){
                                                ...
                                                error: function (jqXHR, textStatus, errorThrown) {
                                                  ...
                                                  success: function (data) {
                                                    ...
                                                    // get object id only
                                                    progressive_load_data(data_only)  
            ...
            function progressive_load_data(_objectid_response) {
              ...
              console.log('_objectid_response', _objectid_response)
                ...
                /**/
                    ...
                    if (_objectid_array.length > maxCountBrowserDisplay){
                          _objectid_array = _objectid_array.slice(0, maxCountBrowserDisplay)             
                    }
                ...
                // post
                post_url_featureByObjectid = _url + '/'+  _layer_id + '/query'
                post_form_data = '?returnGeometry=true'
                post_form_data += '&outSR=4326'
                post_form_data += '&f=pjson'
                post_form_data += '&outFields=*'
                

                
                get_url_batched_array = []
                postFormData_array = [] // post
                for (let j = 0;j < batch_of_chunk_2D_objectid_array.length; j += 1) {
                    ...
                    // post
                    postFormData_array.push(post_form_data + '&objectIds=' + objectid_string_by_comma)

                }//for

                console.log('get url batched array', get_url_batched_array)
                console.log('post url batched postData array', postFormData_array)


                on_map_total = 0
                $("#loading-feature").html(on_map_total)
                // before add new batch data, clear all previous data
                ...
                switch (sync_async){
                  ...
                  case "sync-get":
                    noWait_Async_get_ajax(get_url_batched_array);
                  break;

                  case "async-get":
                    waitSyncGet_ajax(get_url_batched_array);
                  break;

                  case "sync-post":
                    noWaitAsync_post_ajax(post_url_featureByObjectid, postFormData_array);
                  break;

                  case "async-post":
                    waitSyncPost_ajax(post_url_featureByObjectid, postFormData_array);
                  break;
            ...
            var ___XMLHttpRequest_progressiveData_get_array = []
            function noWait_Async_get_ajax(_url_batched_array) {



              //  ***** Only for batch, abort previously ajax call ***** 
                for (let i = 0; i < ___XMLHttpRequest_progressiveData_get_array.length; i++) {
                  if (___XMLHttpRequest_progressiveData_get_array[i]){
                      if (typeof ___XMLHttpRequest_progressiveData_get_array[i].abort !== "undefined"){
                          // abort will cause jsonp call back error(normal, no need fix) :  Uncaught ReferenceError jqueryxxxxx_xxxxx  is not defined
                          ___XMLHttpRequest_progressiveData_get_array[i].abort()  
                          console.log('abort sync get ajax array index', i)
                      }
                  }
                }//for
                ___XMLHttpRequest_progressiveData_get_array = []
              //  ******* end ***** Only for batch, abort previously ajax call   *****  


              for (const _apiURI of _url_batched_array) {

                try {
                
                        console.log('sync get ajax ---> : ', _apiURI);
                        // jsonp  
                        $.ajax({  
                          timeout: _timeout,
                          type: 'GET',
                          ...
                          data: {},
                           ...
                           beforeSend:  function( jqXHR, settings ){
                                                          ...
                                                          ___XMLHttpRequest_progressiveData_get_array.push(jqXHR)
                                                        },
            ...
            // a s y n c - a w a i t - can not do abort, when user continue pan-zoom, load+ total number will more than actual number because last time old ajax now 
            // loaded, also add to new list
            async function waitSyncGet_ajax(_url_batched_array) {

              for (const _apiURI of _url_batched_array) {
                try {
                  
                        console.log('async get ajax ---> : ', _apiURI);
                        // jsonp  
                        ...
                        if (typeof response === 'object') {
                         ...
                         console.log('async get response : ', data_only);
                        add_data_maps(data_only) 
            ...
            var ___XMLHttpRequest_progressiveData_post_array = []
            function noWaitAsync_post_ajax(_apiURI, _postFormDataArray) {



              //  ***** Only for batch, abort previously ajax call ***** 
                for (let i = 0; i < ___XMLHttpRequest_progressiveData_post_array.length; i++) {
                  if (___XMLHttpRequest_progressiveData_post_array[i]){
                      if (typeof ___XMLHttpRequest_progressiveData_post_array[i].abort !== "undefined"){
                          // abort will cause jsonp call back error(normal, no need fix) :  Uncaught ReferenceError jqueryxxxxx_xxxxx  is not defined
                          ___XMLHttpRequest_progressiveData_post_array[i].abort()  
                          console.log('abort sync post ajax array index', i)
                      }
                  }
                }//for
                ___XMLHttpRequest_progressiveData_post_array = []
              //  ******* end ***** Only for batch, abort previously ajax call   *****  




              for (const _postFormDataString of _postFormDataArray) {

                try {
                
                        console.log('sync post ajax ---> : ', _apiURI);
                        // jsonp  
                        $.ajax({  
                          timeout: _timeout,
                          type: 'POST',
                          dataType: 'jsonp',
                          data: _postFormDataString,

                       
                           beforeSend:  function( jqXHR, settings ){

                                                          ___XMLHttpRequest_progressiveData_post_array.push(jqXHR)
                                                        },
                          ...
                          success: function (data) {
                                          ...
                                          if (typeof data === 'object') {
                                           ...
                                           console.log('sync post response : ', data_only);
                                          add_data_maps(data_only) 
            ...
            // a s y n c - a w a i t - can not do abort, when user continue pan-zoom, load+ total number will more than actual number because last time old ajax now 
            // loaded, also add to new list
            async function waitSyncPost_ajax(_apiURI, _postFormDataArray) {

               for (const _postFormDataString of _postFormDataArray) {
                try {
                  
                        console.log('async post ajax ---> : ', _apiURI);
                        // jsonp  
                        var response = await $.ajax({  
                          timeout: _timeout,
                          type: 'POST',
                        ...
                        console.log('async post response : ', data_only);
                        add_data_maps(data_only) 
            ...
            // only add new data, remove data at previous get map bound
                                    ...
                                    //_geojson_object = Terraformer.arcgisToGeoJSON(arcgis_feature_Set,'OBJECTID_1')
                                          ...
                                          /**/
                                                  ...
                                                  map.data.addGeoJson(_geojson_object);
                                                   ...
                                                   on_map_total +=  _geojson_object.features.length
                                                   $("#loading-feature").html(on_map_total)


                                                   console.log('add geojson object to map',_geojson_object.features.length,  _geojson_object)
            ...
            function init_progressive(){
              ...
              $("input[type=radio][name=sync_async_radio][value=" + sync_async + "]").prop('checked', true);
              // radio change event
              $("input[type='radio'][name='sync_async_radio']").change(function(){
                sync_async = $("input[type='radio'][name='sync_async_radio']:checked").val();
                console.log(" sync_async : --  ", sync_async);
                update_url_parameter('sync_async', sync_async);
               ...
               $('#progressive_interval_label').text(batch_count);
               $('#progressive_interval_range').val(batch_count);
  
               $('#progressive_interval_range').on('change', function() {
                    batch_count = parseInt($('#progressive_interval_range').val());
                    $('#progressive_interval_label').text(batch_count);
                    update_url_parameter('progressive', batch_count);
                    get_map_bound()
               });
