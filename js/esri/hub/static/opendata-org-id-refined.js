...
// step 4:  json.zip for opendata-refined.html?file=json.zip

// step 5: use refined  http://localhost:10/json2tree/esri/hub/static/opendata-refined.html?file=opendata_refined_us_only.zip&filter_by=
     ...
     async function rendering_json_to_html(json) {
          ...
          for (var i = 0; i < _results.length; ++i){  
            ...
            // ******************** end *************** only      calculate for hub.arcgis.com , opendata.arcgis.com    only   ****************************
              ...
              if (_results[i].esri_hosted_arcgis_server_home_folder){
               ...
               // ------ end fix bug,
                _open_link = _link_protocol + "//" + window.location.host +  '/json2tree/esri/server/folder2.html?url=' +  _results[i].esri_hosted_arcgis_server_home_folder + '&org=' + encodeURIComponent(hub_orgName).replace(/[!'()*]/g, escape);
      ...
      function build_json_for_csv(_raw_json){
        ...
        for (let i = 0; i < _raw_json.length; i++) {
              ...
              single_hub_item['organization_name'] =  _results[i].organization_name;
