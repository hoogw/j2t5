      ...
      dom_ready_dojo();
       ...
       var map = new Map({
        ...
        /**/
         ...
         // - - - esriSearchWidget customized by each field - - - 
         // default search widget remove here and move to  init_search_widget_customized_by_each_field
      ...
      function search_layer_now(){
              ...
              // search logic here
                       ...
                       if (searchText){
                                              ...
                                              // not use, because, "region 10", will find all 'region' 
                                              //_where_condition = build_where_condition_multiple_keyword(searchText, ___where_clause_operator)
                                              // in use, 
...
$('#info-window-div').html("")
      ...
      /**/
        ...
        /**/    
          ...
          var search_key_word = ''
          var sourceFieldName
          var current_activeSourceIndex



              async function init_search_widget_customized_by_each_field(){

                var search_widget_source_array = []
                var default_rooftop_source = {
                ...
                // do not use, because this will not fly to, zoom to
                //search_widget_source_array.push(default_rooftop_source)  



       // . . . for loop to create each source for each field  . . . 
       /**/
                // depends on: whether add "default_rooftop_source"  or includeDefaultSources: true/false, source index = 1 or  0,
                var source_index = 0
                var source_indexed_fieldName_object = {}  // { sourceIndex: field name}

                var each_field_source
                var each_field_name
               
                console.log(' string text based source,  ', _feature_attributes_string)
                console.log(' number, integer,double, based source,  ', _feature_attributes_integer)

                /**/
                //  - - 1 - - field is string type 
                for (let s = 0; s < _feature_attributes_string.length; s++){
                 
                          each_field_name = _feature_attributes_string[s]
                          console.log('suggest . . . string . . . each field name',  each_field_name)

                          source_index += 1
                          source_indexed_fieldName_object[source_index] = each_field_name
                          
                          // layer search source api https://developers.arcgis.com/javascript/latest/api-reference/esri-widgets-Search-LayerSearchSource.html    
                          // sample https://developers.arcgis.com/javascript/latest/sample-code/widgets-search-multiplesource/                        
                          each_field_source = new SearchSource({

                                                  layer: backgroundFeatureLayer,
                                                  displayField: each_field_name,    // "", emplty defaults to FeatureLayer.displayField
                                                  searchFields: [each_field_name],  // [], emplty will defaults to FeatureLayer.displayField

                                                  // suggestion
                                                  suggestionsEnabled: true,
                                                  //suggestionTemplate: '{' + each_field_name + '}',  // not use,   "{ZoneCode}, : {ZoneName}", 
                                                  maxResults: 6,
                                                  maxSuggestions: 6,
                                                  ...
                                                  exactMatch: false,
                                                  outFields: ["*"],
                                                  name: each_field_name,  // not work, FeatureLayer.displayField,
                                                  placeholder: each_field_name + ' (text)',


                                                  /* not use, marker, only address search use marker, polygon, line, point just use highlighted symbol
                                                  resultSymbol: {
                                                    type: "picture-marker",  // autocasts as new PictureMarkerSymbol()
                                                    url: window.location.protocol + "//" + window.location.host + "/json2tree/favicon.ico",
                                                    size: 24,
                                                  ...
                                                  */
                                                  ...
                                                  getSuggestions: (params) => {
                                                    ...
                                                    console.log('get Suggestions params : ', params)
                                                    var suggest_key_word = params.suggestTerm
                                                    console.log('suggest key word : ', suggest_key_word)

                                                    sourceFieldName = source_indexed_fieldName_object[params.sourceIndex]
                                                    console.log('suggest source field name : ', sourceFieldName)

                                                    
                                                    var where_clause_for_filterValueList = "LOWER(" + sourceFieldName + ") like '%" + suggest_key_word.toLowerCase() + "%' "
                                                    //var where_clause_for_filterValueList = "LOWER(" + 'ZoneCode' + ") like '%" + suggest_key_word + "%' "
                                                    var encoded_where_clause_for_filterValueList = encodeURIComponent(where_clause_for_filterValueList)


                                                    var search_suggestion_url = ''
                                                    search_suggestion_url += background_layer_url 
                                                    search_suggestion_url += '/query?returnGeometry=false&f=pjson'
                                                    search_suggestion_url += '&outFields=' + sourceFieldName
                                                    search_suggestion_url += '&where=' + encoded_where_clause_for_filterValueList

                                                    // warning: distinct works only if the supportsAdvancedQueries property of the layer is true
                                                    search_suggestion_url += '&returnDistinctValues=true'  

                                                    // never use this, this will cause error(some time do not support pagination) on v10.8.1 and older version
                                                    if (_supportsPagination){
                                                      // never use this, this will cause error(some time do not support pagination) on v10.8.1 and older version
                                                      //search_suggestion_url += '&resultOffset=0&resultRecordCount=10'
                                                    } else {
                                                      // not support pagination, do not use result off set 
                                                    } 

                                                    console.log('search suggestion url : ', search_suggestion_url)

                                                  
                                                    return esriRequest(search_suggestion_url, {
                                                      //responseType: "json"
                                                    }).then((results) => {
                                                      // Return Suggestion results to display in the Search widget
                                                      console.log('Return Suggestion results : ', results)
                                                
                                                      // remove duplicated value
                                                      var unique_value_array = []
                                                      results.data.features.map((feature) => {

                                                        console.log('Return Suggestion each feature (array) item : ', feature)
                                                        var possible_distinct_value = feature.attributes[sourceFieldName]
                                                        if (unique_value_array.includes(possible_distinct_value)){
                                                          // duplicated value, skip, nothing to do
                                                        } else {
                                                          unique_value_array.push(possible_distinct_value)
                                                        }

                                                      });

                                                      console.log('unique value array : ', unique_value_array)
                                                      
                                                      return unique_value_array.map((_uniqueSuggestValue) => {
                                                                  return {
                                                                    key: sourceFieldName,
                                                                    text: _uniqueSuggestValue,  //feature.attributes.ZoneCode,
                                                  ...
                                                  getResults: (params) => {
                                                                              ...
                                                                              console.log("get results params", params); 

                                                                              search_key_word = params.suggestResult.text
                                                                              console.log('search key word : ', search_key_word)

                                                                              // only always works
                                                                              sourceFieldName = source_indexed_fieldName_object[params.sourceIndex]
                                                                              // sometime not work, it is null
                                                                              //sourceFieldName = source_indexed_fieldName_object[params.suggestResult.sourceIndex]
                                                                              // sometime not work, it is null
                                                                              //sourceFieldName = params.suggestResult.key
                                                                              console.log('search source field name : ', sourceFieldName)

                                                                              var where_clause_for_filterValueList = "LOWER(" + sourceFieldName + ") = '" + search_key_word.toLowerCase() + "' "
                                                                              var encoded_where_clause_for_filterValueList = encodeURIComponent(where_clause_for_filterValueList)

                                                                              var search_result_url = ''
                                                                              search_result_url += background_layer_url 
                                                                              search_result_url += '/query?returnGeometry=true&f=pjson'
                                                                              search_result_url += '&outFields=*'

                                                                              // warning:  must always specify spatial reference 4326, otherwise, will mess up
                                                                              search_result_url += '&outSR=4326&where=' + encoded_where_clause_for_filterValueList

                                                                              console.log('search result url : ', search_result_url)


                                                                              return esriRequest(search_result_url, {
                                                                                // https://developers.arcgis.com/javascript/latest/api-reference/esri-request.html#RequestOptions
                                                                                //responseType: "json"
                                                                              }).then((results) => {
                                                                                // Parse the results of your custom search
                                                                                console.log('Return search results : ', results)

                                                                                var __displayFieldName = results.data.displayFieldName
                                                                                var geometry_type = results.data.geometryType
                                                                                ...
                                                                                var searchResults = results.data.features.map((feature) => {
                                                                                                                ...
                                                                                                                console.log(' each feature in search result : ', feature)
                                                                                                                // Create a highlight Graphic the Search widget can display ( search widget view model)
                                                                                                                ...
                                                                                                                if (geometry_type == 'esriGeometryPolygon'){
                                                                                                                      ...
                                                                                                                      // raw search results is just array of feature, must use this map function to re-format as { extent, feature(graphic), name(label)} 
                                                                                                                      return {
                                                                                                                                          //extent: graphic_as_highlight.extent,
                                                                                                                                          feature: graphic_as_highlight,
                                                                                                                                          name: feature.attributes[__displayFieldName]
                                                                                                                              };
                                                                                                                }// if polygon       
                                                                                                                
                                                                                                                if ((geometry_type == 'esriGeometryPoint') || (geometry_type == 'esriGeometryMultipoint')){
                                                                                                                      //  point highlight graphic only
                                                                                                                      graphic_as_highlight = new Graphic({
                                                                                                                                  geometry: {
                                                                                                                                    type: "point",
                          ...
                          search_widget_source_array.push(each_field_source) 
                ...
                /**/
                //   - - 2 - - field is number type  - -  only for number, integer, or double 
                for (let n = 0; n < _feature_attributes_integer.length; n++){

                  each_field_name = _feature_attributes_integer[n]
                  console.log('suggest  1 2 3 number 3 2 1 each field name',  each_field_name)
                  ...
                  each_field_source = new SearchSource({
                                          ...
                                          outFields: ["*"],
                                          name: each_field_name,  // not work, FeatureLayer.displayField,
                                          placeholder: each_field_name + ' (number)',
                                          ...
                                          */
                                          ...
                                          getSuggestions: (params) => {
                                            ...
                                            // only for number, integer, or double 
                                            var where_clause_for_filterValueList = "CAST(" +  sourceFieldName + " as varchar(255)) like '%" + suggest_key_word + "%' "
                                            //var where_clause_for_filterValueList = "LOWER(" + 'ZoneCode' + ") like '%" + suggest_key_word + "%' "
                                            var encoded_where_clause_for_filterValueList = encodeURIComponent(where_clause_for_filterValueList)
                                            ...
                                            return esriRequest(search_suggestion_url, {
                                              ...
                                              results.data.features.map((feature) => {
                                                ...
                                                // only for number, integer, or double
                                                var possible_distinct_value_as_number = feature.attributes[sourceFieldName]
                                                possible_distinct_value = possible_distinct_value_as_number.toString();
                                          ...
                                          getResults: (params) => {
                                                                      ...
                                                                      // only for number, integer, or double 
                                                                      var where_clause_for_filterValueList = sourceFieldName + " = " + search_key_word
                                                                      var encoded_where_clause_for_filterValueList = encodeURIComponent(where_clause_for_filterValueList)
  ...
  /**/
  //  . . .    end   . . . for loop to create each source for each field  . . . 
                ...
                searchAddressWidget = new Search({
                    ...
                    sources:   search_widget_source_array
                });
                ...
                searchAddressWidget.on("search-clear", function(event){

                    console.log(' you click clear x on esri search widget bar', event)
                    ...
                    // also filter out other both on feature layer and map image layer
                    if (background_type == 'data'){
                      // search only,  
                              filter_featureLayerView_by_keyword(sourceFieldName, search_key_word)
                    } else if (background_type == 'image'){
                      // search only
                              filter_mapImageLayer_by_keyword(sourceFieldName, search_key_word)
                    }
                ...
                searchAddressWidget.on("search-complete", function(event){
                  ...
                  console.log("search complete event, total number of result feature (only if return all result, not apply if only return 1st result): ", event.results[0].results.length);
                  console.log("search complete event, result feature array : ", event.results[0].results);
                  console.log("search complete event, active source index : ", event.activeSourceIndex);

                  current_activeSourceIndex = event.activeSourceIndex
                  ...
                  // get 1st result is only for zoom to first result, user can click popup window's other result to zoom to other results
                  searchResults_1st_highlight_graphic = event.results[0].results[0].feature
                  console.log('get 1st result is only for zoom to first result, user can click popup window other result to zoom to other results : ', searchResults_1st_highlight_graphic)
                  //view.graphics.add(searchResults_1st_highlight_graphic)

                  // highlight all results, but only zoom to first results, 
                  var searchResults_array = event.results[0].results
                  searchResults_highlight_graphic_array = []
                  for (let i = 0; i < searchResults_array.length; i++) {
                    searchResults_highlight_graphic = searchResults_array[i].feature 
                    searchResults_highlight_graphic_array.push(searchResults_highlight_graphic)
                    view.graphics.add(searchResults_highlight_graphic)
                  }//for


                  if (current_activeSourceIndex == 0){
                       // for search place,address only, nothing to do, will auto zoom to result
                  } else {
                              // for search layer field only, search point,polygon,line, layer, will not auto go to, must manually go to.
                              view.goTo(searchResults_1st_highlight_graphic);
                              ...
                              /*
                              if ((searchResults_1st_highlight_graphic.geometry.extent == null) || 
                                  (searchResults_1st_highlight_graphic.geometry.type == 'point')){
                                      var __lat = searchResults_1st_highlight_graphic.geometry.latitude
                                      var __long = searchResults_1st_highlight_graphic.geometry.longitude
                                      var __zoom = default_zoom_level_for_point
                                      console.log('type is point, extent is null, zoom To Lat Long, manually goto  ', __lat, __long, __zoom)
                              } else {
                                      // polygon and line, extent is not null, works fine with goTo
                                      console.log('type is line or polygon, extent is NOT null, manually goTo : ')
                              }
                              */
                  ...
                  /**/
                        ...
                        for (var i = 0; i < searchResults_array.length; i++){
                          searchResults_array_properties_only.push(searchResults_array[i].feature.attributes) 
                        } 
                        ...
                        show_search_Tab(searchResults_array_properties_only, search_key_word)
                        // ==== end ======= show json on search tab result panel ===========
                  /**/
                  //     ... - .. - ..  end .. - .. - ... zoom 2 feature   ... - .. - .. 
                  /**/
        ...
        /**/ 
                    ...
                    // search only, Filter feature using map image layer
                    function filter_mapImageLayer_by_keyword(current_selected_field_name, current_selected_field_value){
                      /*
                      ...
                      */
                          ...
                          if (current_selected_field_name == ''){
                    ...
                    // search only,  Filter feature using feature layer
                    function filter_featureLayerView_by_keyword(current_selected_field_name, current_selected_field_value){
        ...
        /**/
        // ... end ...  - - - esriSearchWidget customized by each field  - - -
        /**/
                        ...
                        function zoom_to_feature(esri_highlighted_graphic,  _searchResult_serial_number, _action){
                          ...
                          if ((_action) || (zoom2feature_yesNo == 'donot_zoom2feature')){
                            ...
                            // nothing to do, do not zoom to anywhere, just keep still
                                            ...
                                            console.log('zoom2feature zoom2feature_noMoreThan  zoomLevel ', zoom2feature_noMoreThan)
                                            if (esri_highlighted_graphic.geometry.type == 'point'){
                                              view.zoom = zoom2feature_noMoreThan
                                            }
                                          ...
                                          view.goTo(esri_highlighted_graphic);
                                          console.log('zoom2feature fixed_zoom_level  zoomLevel ', zoom2feature_zoomLevel)
                                          view.zoom = zoom2feature_zoomLevel
          ...
          await get_feature_attributes(background_mapserver_url, layer_id)
            ...
            init_print() //  ---  print   --- 
             ...
             /**/
                            ...
                            init_search_widget_customized_by_each_field()
                            /**/
                            // ... end ...  - - - esriSearchWidget customized by each field  - - -
                            /**/
