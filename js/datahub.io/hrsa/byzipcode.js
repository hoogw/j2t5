...
require([
  ...
  "esri/Graphic",
      ...
      var zipcode_layer_url = ___url_string + '/9' 

      var zipcode_field_name = 'POSTCODE'
    ...
    var mapimagerlayer_sub_layer // filter map image layer, use layer definition expression
      ...
      var zipcodeMapImageLayer
      var zipcodeFeatureLayer
...
/* .... ... ... external function ... ... ... */     
                        ...
                        if (indexOfFirst > -1) {
                          ...
                          */
                          ...
                          b.addEventListener("click", function(e) {
                              ...
                              console.log('auto complete * * * * click event * * * * need to auto search zipcode * * * * ')
                              $('#possible_btn_span').text('')
                              search_zipcode()
                ...
                /*initiate the autocomplete function on the "myInput" element, and pass along the zipcode_array array as possible autocomplete values:*/
                autocomplete(document.getElementById("zipcode_text"), zipcode_array);

        /* -------- end ---------- autocomplete --------------*/



    function zoom_to_zipcode(_zip_code){
      ...
      $('#result_div').html('')
                ...
                // Add zipcode layer
                remove_current_layer(zipcodeFeatureLayer)

                /*  
                //not use image layer, because you can't change color and label of a image layer, instead, you should use feature layer
               
                                  zipcodeMapImageLayer = new MapImageLayer({
                                                                        url: ___url_string,
                                                                        sublayers: [{
                                                                            id: 9,
                                                                            title: _zip_code,

                                                                            // not use, because multiple county could have same name, 'orange county'
                                                                            //definitionExpression: level2_field_name +"='" + workonthiskeyword + "'",
                                                                            definitionExpression: zipcode_field_name +'=\'' + _zip_code + '\'' ,
            ...
            /*
               // not use, this will add label to entire layer data, not just filtered result
               const labelClass = {
            ...
            */
                ...
                zipcodeFeatureLayer = new FeatureLayer({
                                        url: zipcode_layer_url, 
                ...
                // not use, this will add label to entire layer data, not just filtered result
                //map.add(zipcodeFeatureLayer)


                  
                const zipcodeQuery = {

                  where: zipcode_field_name + '=\'' + _zip_code + '\'',  // Set by select element
                ...
                zipcodeFeatureLayer.queryFeatures(zipcodeQuery)
                ...
                zipcodeFeatureLayer.queryExtent(zipcodeQuery).then(function (results) {
                                        console.log(' then results extent ', results.extent)
    ...
    function displayResults(results) {
      ...
      view.goTo(results.features);
          ...
          results.features.map((feature) => {
            ...
            // results is Graphic object,  https://developers.arcgis.com/javascript/latest/api-reference/esri-Graphic.html
            feature.symbol = symbol;
            feature.popupTemplate = popupTemplate;

            console.log(' result feature is here ', feature)
            return feature;
          });
    ...
    function search_zipcode(){
      ...
      var raw_zipcode = $('#zipcode_text').val();
      console.log('raw zip code', raw_zipcode)

      if ((raw_zipcode.indexOf(',') > -1)){
                  // '17, Northeast Fresno County, CA'
                  zipcode = raw_zipcode.substr(0, raw_zipcode.indexOf(','))
      } else {
                  // 08879
                  zipcode = raw_zipcode
      }
      ...
      console.log('extracted real zip code', zipcode)

      update_url_parameter('search', zipcode);
      
      zoom_to_zipcode(zipcode)
    ...
    function pre_select(){
        ...
        // pre-search
        search = param_search
        console.log('init search zip code', search)
        if (search){
          zoom_to_zipcode(search)
          $( "#zipcode_text" ).val(search)
        }
    ...
    function init_zipcode(){
      ...
      $( "#zipcode_text" ).on( "search", function(){
        search_zipcode()
      }); 

      $( "#zipcode_text" ).on( "keyup", function(){
        $('#result_div').html('')
        zipcode = $('#zipcode_text').val();
        console.log('user just type zip code, do not search yet, only update param', zipcode)
        update_url_parameter('search', zipcode);
      }); 
