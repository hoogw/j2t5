  ...
  "esri/core/reactiveUtils",
            ...
            reactiveUtils,
              ...
              ui: {
                ...
                popup: {
                   // geojson should not disable popup, because geojson could be line and point mix together, geometry type will be polyline
                   // renderer will use polyline, than those point will not be able to get tigger hover or click, but ESRI popup template works on those points.
                   // so keep esri popup template.
...
// - - - - Create a FeatureLayer from a shapefile  - - - - https://developers.arcgis.com/javascript/latest/sample-code/layers-featurelayer-shapefile/
      ...
      function addShapefileToMap (featureCollection) {
        ...
        var layers = featureCollection.layers.map((layer) => {
                    ...
                    console.log('fields definition is array of FIELD object ', fields_definition)
      ...
      async function dom_ready_dojo(){
       ...
       document.getElementById("uploadForm").addEventListener("change", (event) => {
        ...
        if (fileName.indexOf(".geojson") !== -1) {//is file a zip - if not notify user
          ...
          document.getElementById('upload-status').innerHTML = '<p style="color:red">Add GeoJSON as xxx.geojson file</p>';
        }
      });
