...
//       - -  -  -  geojson   - -  -  - // only for added geojson reference layer
 
var file_fakepath
var fileName
// only for added geojson reference layer
var _added_geojson_reference_layer
var _added_geojson_object

// only for added geojson reference layer
function add_geojson_google(______full_geojson_string_____){
   
    // this is whole uploaded geojson file
    _added_geojson_object = JSON.parse(______full_geojson_string_____)
               
    // everytime upload a new geojson, need pan to real location
      pan_to_geojson(_added_geojson_object)

      // only for added geojson reference layer
      _added_geojson_reference_layer  = map.data.addGeoJson(_added_geojson_object);
} 
...
//      - -  -  -   end    - -  -  -  geojson   - -  -  -  
          ...
          /**/
          // -- -- --  split address -- -- -- only for  selected feature array  -- -- -- 
          /**/
          ...
          var feature_zip
            ...
            async function init_split_address(){
                ...
                $("#split-address-button").on("click", function() {
                  ...
                  // current_addressField
                  for (let i = 0; i < selected_feature_array.length; i++) {
                    feature_element = selected_feature_array[i]
                    ...
                    selected_feature_array[i]["properties"]["feature_streetNumber"] = feature_streetNumber
                    selected_feature_array[i]["properties"]["feature_streetPrefix"] = feature_streetPrefix
                    selected_feature_array[i]["properties"]["feature_streetNameOnly"] = feature_streetNameOnly
                    selected_feature_array[i]["properties"]["feature_streetType"] = feature_streetType
                    selected_feature_array[i]["properties"]["feature_city"] = feature_city
                    selected_feature_array[i]["properties"]["feature_state"] = feature_state
                    selected_feature_array[i]["properties"]["feature_zip"] = feature_zip
                    
                  
                  }//for

                  console.log('after split address, now all feature array is ',  selected_feature_array)



                  // do not refresh map with new address components
...
/**/
        ...
        function manually_adjust_max_square_edge(){
          $("#square_edge_range").attr("max", max_square_edge);
          $("#square_edge_range").val(current_square_edge);

          var html_currentValue = '<span font-size:13.6px; style="font-weight:800">' + current_square_edge + ' </span>meters'
          html_currentValue += '<span font-size:16.6px; style="font-weight:800">(' + current_square_edge_miles + ' </span>miles)'
          $("#square_edge_value_label").html(html_currentValue);

          var html_maxValue = '<span style="font-size:10.7px">Max square edge: <span style="font-size:16.6px; font-weight:800">' + max_square_edge + ' </span>meters'
          html_maxValue += '<span style="font-size:12.6px; font-weight:800">(' + max_square_edge_miles + ' </span>miles)'
          html_maxValue += ' otherwise square expand beyond current map view'
          $('#manual_square_edge_div').html(html_maxValue);

      }
        

        function get_square_edge_in_map_bound(){
          ...
          if (vertical_distance <= horizontal_distance){
            min_square_edge = vertical_distance
          } else {
            min_square_edge = horizontal_distance
          }
        ...
        function drawing_shape(_squareEdgeMeter, _centerLngLatPoint){
          ...
          google_shape = new google.maps.Rectangle({
            strokeColor: "#FF0000",
            strokeOpacity: 0.8,
            strokeWeight: 2,
            fillColor: "#FF0000",
        ...
        async function select_feature_intersect_with_square(_squareEdgeMeter, _centerLngLatPoint){

                  console.log('select feature intersect with square at _squareEdgeMeter, _centerLngLatPoint', _squareEdgeMeter, _centerLngLatPoint)
                  ...
                  var SWlong = mid_west_lng;
                  var SWlat = mid_south_lat;
                  var NElong = mid_east_lng;
                  var NElat = mid_north_lat;
        ...
        function add_ground_overlay_listener_click(){
          ...
          var listener_ground_overlay_click =  groundoverlay.addListener('click', function(click_event) {   
            ...
            drawing_shape(current_square_edge, click_lat_lng_point)
