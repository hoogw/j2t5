...
require([
  ...
  "esri/Graphic",
          ...
          // for zip code only, buffer size and relationship is special for zip code 
          var negative_buffer = -1
          ...
          var zip_layer_url = ___url_string + '/9' 


          // level 1 is state name
          var level1_field_name = 'ADMIN2'
          var level1_url = zip_layer_url + '/query?outFields='
          ...
          // level 2 is county
          var level2_field_name = 'ADMIN3'
          var level2_url = zip_layer_url + '/query?outFields='
          ...
          // level 3 is zip
          var level3_field_name = 'POSTCODE'
          var level3_url = zip_layer_url + '/query?outFields='
              level3_url += level3_field_name + '&f=pjson&returnGeometry=false&returnCountOnly=false&returnDistinctValues=true&where='
              level3_url += level1_field_name + '=\''
          var raw_distinct_level3_value
          var level3_value_array = []
          var clicked_level3_value



          var current_selected_level_1 = 'showall'
          var current_selected_level_2 = 'showall'
          var current_selected_level_3 = 'showall'
          var current_selected_level_1_tag_id = -2
          var current_selected_level_2_tag_id = -2
          var current_selected_level_3_tag_id = -2
...
/**/
  ...
  /* .... ... ... external function ... ... ... */     
              ...
              async function work_on_value(workonthiskeyword, index){
                ...
                console.log(' work_on_value ', workonthiskeyword, index, level3_field_name )

                // mark level 3 selection
                current_selected_level_3 = index
                current_selected_level_3_tag_id = index
                $('.level3value_class').removeClass('selected_style')
                $('#level3value_tag_' + index).addClass('selected_style')
                update_url_parameter('select_level3value', current_selected_level_3);


                //Add county layer
                remove_current_layer(zipMapImageLayer)
                ...
                //Zoom to zip
                zipMapImageLayer = new FeatureLayer({
                    url: zip_layer_url
                });
                ...
                const zipQuery = {
                                      ...
                                      // zip code does not have duplicate, only use level3=zip, (not like county, so no need  use, leve1=state, level2=county level3=zip, )
                                      where: level3_field_name + "='" + workonthiskeyword + "'",
                ...
                zipMapImageLayer.queryFeatures(zipQuery)
                ...
                zipMapImageLayer.queryExtent(zipQuery).then(function (results) {
              ...
              // only for zip code layer (popup template) 
              function displayResults(results) {
                ...
                view.goTo(results.features);
                      ...
                      const popupTemplate = {
                        ...
                        content: "{ADMIN3} County<br>  {ADMIN2}&nbsp;{STATE}<br> state-fips:{STFIPS} &nbsp; city-fips:{CTYFIPS} <br> {PA_NAME}<br> "
                      };
              ...
              function testRelationship(results, buffer_distance, buffer_unit){
                ...
                */
                              const backgroundQuery_1 = {
                                ...
                                spatialRelationship :  "within",  //  "intersects",   "overlaps",    "contains", 
                                outFields: ['*'], // Attributes to return
      ...
      /*
              ...
              function clear_level3(){

                $('.level3value_class').removeClass('selected_style')
                $('#level3value_tag_-1').addClass('selected_style')

                update_url_parameter('select_level3value', -1);
                $("#result_div").html('');
              ...
              async function build_level3_list(filterbykeyword, index){
                ...
                update_url_parameter('select_level3value', -1);
                    ...
                    console.log('build level 3 html by value', filterbykeyword, index, level3_field_name)

                    console.log('level3_url  before add level 1,2 where filter', level3_url)
                    var this_level3_url =  level3_url + encodeURIComponent(clicked_level1_value) 
                        this_level3_url += '\' AND ' + level2_field_name + '=\'' + encodeURIComponent(filterbykeyword) + '\''
                    console.log('level3_url after add level 1,2 where filter', this_level3_url)

                    raw_distinct_level3_value = await ajax_getjson_common(this_level3_url)
                    raw_distinct_level3_value = convert_to_json(raw_distinct_level3_value)
                    console.log(' raw level 3 ', raw_distinct_level3_value)

                    level3_value_array = []
                   
                    var _level_3_value
                    for (let j = 0; j < raw_distinct_level3_value.features.length; j++) {
                      _level_3_value = raw_distinct_level3_value.features[j]['attributes'][level3_field_name]
                      level3_value_array.push(_level_3_value)
                     
                    }
                    level3_value_array.sort()
                    console.log(' ordered level 3 array', level3_value_array)
                    ...
                    if (level3_value_array) {
                      if ( level3_value_array.length > 0 ) {
                          
                              value_html += '<ol>';

                              value_html += '<h2 style="display:inline">ZipCode</h2>'
                              value_html += '</br>';
                              value_html +=   '<a  id="level3value_tag_-1" class="level3value_class" href="#">';
                              value_html +=       '<b>clear selection</b>'
                              value_html +=   '</a>';
                              value_html +=   '</br></br>';
                              
                              for (var r = 0; r < level3_value_array.length; r++) { 
                                        // ======  ===== =======    distinct  ======  ===== ======= 
                                        _node_display_text = level3_value_array[r]
                                        _this_field_value = level3_value_array[r]
                            
                                        value_html += '<li>' // css .ordered_list_number{ size font};
                                        value_html +=   '<a id="level3value_tag_' + r + '" class="level3value_class" href="#" index="'  + r + '"   >';
                    ...
                    if (level3_value_array) {
                      if ( level3_value_array.length > 0 ) {
                        ...
                        // clear selection
                        $('#level3value_tag_-1').on('click', function(event){
                          event.preventDefault();
                          clear_level3()
                        })

                        for (var e = 0; e < level3_value_array.length; e++) { 

                          var this_tag_id = '#' + 'level3value_tag_' + e.toString()
                          $(this_tag_id).on('click', function(event){
                              event.preventDefault();
                              clicked_level3_value = $(event.target).text();
                              var tag_index = $(this).attr('index');
                              console.log('click -- level 3 --- ', clicked_level3_value, tag_index,  this);

                              work_on_value(clicked_level3_value, tag_index)
                          })
                ...
                update_url_parameter('select_level2value', -1);
                    ...
                    if (level2_value_array) {
                      if ( level2_value_array.length > 0 ) {
                              ...
                              value_html += '<h2 style="display:inline">County</h2>'
                              value_html += '</br>';
              ...
              async function pre_select(){
                        ...
                        console.log('pre_select  current_selected_level_3 ', current_selected_level_3)
                                        ...
                                        if (current_selected_level_2){
                                          if (current_selected_level_2 == -1){
                                                // nothing to do
                                                      ...
                                                      console.log('pre_select this_tag_id clicked_level2_value ',this_tag_id, clicked_level2_value)

                                                      await build_level3_list(clicked_level2_value, current_selected_level_2)

                                                      if (current_selected_level_3){
                                                        if (current_selected_level_3 == -1){
                                                          // nothing to do
                                                        } else {
                                                                    var this_tag_id3 = '#' + 'level3value_tag_' + current_selected_level_3.toString()
                                                                    clicked_level3_value = $(this_tag_id3).text();
                                                                    console.log('pre_select this_tag_id clicked_level3_value ',this_tag_id, clicked_level3_value)

                                                                    work_on_value(clicked_level3_value, current_selected_level_3)
                                                        }//if level3
                                                      }//if level3
