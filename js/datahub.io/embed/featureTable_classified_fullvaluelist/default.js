...
/**/
      ...
      for (var c = 0; c < _restructured_json.length; c++) {
              ...
              // only for all-in-1, no-paging version
              function display_valueList_count_info(_____fieldName, _____totalCount_of_current_valueList){
                 var _html_for_count = ''
                 _html_for_count +=   'Found <span style="font-size:24px;"><mark>'   + _____totalCount_of_current_valueList + '</mark></span> Possible Unique Value' 
                 _html_for_count +=   '</br>'
                 _html_for_count +=   '<span style="font-size:10px;">Arcgis Server Return Max Record Count '   + maxRecordCount + '</span>'
                 _html_for_count +=   '</br>'  
