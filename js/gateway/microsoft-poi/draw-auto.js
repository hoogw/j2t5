...
/**/
    ...
    /**/
        ...
        function featureClicked(e) {
            ...
            console.log(' you click feature event ', e)


            // must clear previous other clicked highlight
            ...
            empty_info_outline_Tab()
                    ...
                    if (e.shapes && e.shapes.length > 0) {
                        console.log(' you click feature ',  e.shapes[0]);
                        var ___properties = e.shapes[0].properties
                        console.log(' you click ___properties ',  ___properties);
                        show_info_outline_Tab_nonPOI(___properties)
                        featureLayerGeojson_datasource_highlight.add(e.shapes[0])

                        selected_shape = new atlas.Shape(e.shapes[0].geometry)
                        selected_bounds = selected_shape.getBounds();
                         console.log(' selected bounds', selected_bounds)

                        selected_bounds_north = selected_bounds[3];
                        selected_bounds_east = selected_bounds[2];
                        selected_bounds_south = selected_bounds[1];
                        selected_bounds_west = selected_bounds[0];
                        

                         console.log(' selected bounds N E S W ',  selected_bounds_north, selected_bounds_east, selected_bounds_south, selected_bounds_west);
                    }//if
    ...
    async function nearby_poi_auto_grid(_radiusMeter){
       ...
       // default, or cat. is empty, means everything
        ...
        if (microsoft_search_nearby_response.error){
            ...
            // special because it inside for loop, use .html instead of .append
            $('#info-window-div').html("<span style='font-size:large;'>" + _error_message_html + "</span>")
