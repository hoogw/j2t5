...
/**/
        ...
        // blue color
        var all_selected_objectid_array = []
        // red color
        var current_selected_objectid = -1



        function reset_map_style_and_info_window(){

                  var html_all_selected = ''
                  var html_current_highlighted = ''
                  var _index = 0
                  var __layer_name 
                  
                  // clean last time remaining
                  $('#current-highlighted-div').html("");
                  $('#all-selected-div').html("");
                  ...
                  _geojson_object_selected_feature_objectIDasKey = {}
                  _geojson_object_selected_feature_array = []
                  ...
                  map.data.setStyle(function(feature) {
                    ...
                    var this_object_id = feature.getProperty(current_object_id_field_name);   // default is 'OBJECTID'

                    var index = all_selected_objectid_array.indexOf(this_object_id);
                    ...
                    if (index > -1) {
                      ...
                      // - - info window  - -
                          ...
                          _index += 1
                          __layer_name = ''
                          __layer_name += 'OBJECTID ( ' + this_object_id + ' )'
                          __layer_name += '&nbsp;&nbsp;&nbsp;'
                          __layer_name += '<button type="button" id="unselect_btn_' + this_object_id + '">' + '&#x274C; Unselect &nbsp;' + '</button>'
                        
                       //   - - end - - info window  - -



                      if (this_object_id == current_selected_objectid){

                                  // - - info window  - -
                                      // selected, also current highlighted, red color

                                      console.log('before feature to geojson ', feature)
                                      ...
                                      feature.toGeoJson(function(feature_to_geojson){ 
                                            ...
                                            html_current_highlighted += '</br>'   
                                            html_current_highlighted += '<fieldset>'
                                            html_current_highlighted +=     '<legend>' + _index +  ' : ' + __layer_name +'</legend>'
                                            html_current_highlighted +=     '<div class="flex-row  selected_highlighted_style attribute_field_set_style"   id="attribute_field_set_' + this_object_id +  '"   >' // not 0, means need append to existing info window
                                            html_current_highlighted +=         json_flex_tip_viewer(___properties)
                                            html_current_highlighted +=     '<div>'
                                            html_current_highlighted += '</fieldset>'

                                            $('#current-highlighted-div').html(html_current_highlighted);
                                            // only 1 record, can use this, if multiple record, need loop through all of items
                                            addEvent2html(this_object_id)
                                            ...
                                            // - - - - download geojson  - - - - 
                                            /**/
                                            _geojson_object_selected_feature_objectIDasKey[this_object_id] = feature_to_geojson
                                            _geojson_object_selected_feature_array.push(feature_to_geojson)
                                            /**/
                                            //  - - - -  end  - - - -  download geojson   - - - - 
                                            /**/
                            ...
                            // - - info window  - -
                                // selected, but not current highlighted, blue color 
                                console.log('before feature to geojson ', feature)
                                // get feature's all property  https://developers.google.com/maps/documentation/javascript/reference/data#Data.Feature
                                feature.toGeoJson(function(feature_to_geojson){ 
                                      ...
                                      html_all_selected += '</br>'   
                                      html_all_selected += '<fieldset>'
                                      html_all_selected +=     '<legend>' + _index +  ' : ' + __layer_name +'</legend>'
                                      html_all_selected +=     '<div class="flex-row attribute_field_set_style"   id="attribute_field_set_' + this_object_id +  '"   >' // not 0, means need append to existing info window
                                      html_all_selected +=         json_flex_tip_viewer(___properties)
                                      html_all_selected +=     '<div>'
                                      html_all_selected += '</fieldset>'


                                      $('#all-selected-div').html(html_all_selected);
                                      // multiple record, need loop through all of items
                                      for (let i = 0; i < all_selected_objectid_array.length; i++) {
                                        addEvent2html(all_selected_objectid_array[i])
                                      }
                      ...
                      };//if this object id is currrent highlight object id


                    } else{
                              //  not selected, must set to yellow
                              return {
                                // icon only affect point 
                                icon        : {
                                                  path: google.maps.SymbolPath.CIRCLE,
        ...
        function addEvent2html(___objectID_as_Key){
            ...
            $("#unselect_btn_" + ___objectID_as_Key ).on('click', function(){
                        ...
                        console.log(" you click unselect 1 item , before delete, current selected id is ", all_selected_objectid_array)

                        var index = all_selected_objectid_array.indexOf(_unselect_object_id);

                        if (index > -1) {

                                    // already exist, unselect it, remove from array pool
                                    // only splice array when item is found
                                    all_selected_objectid_array.splice(index, 1); // 2nd parameter means remove one item only

                                           
                        } else{

                                    // new object id selected, add to array pool
                                   console.log('something wrong, unselect button obj id are not on the all selected array', _unselect_object_id)
                        }

                        reset_map_style_and_info_window()
  ...
  /**/
          ...
          $( "#UnselectRemoveAll_btn" ).on( "click", function() {
            ...
            // blue color
            all_selected_objectid_array = []
            // red color
            current_selected_objectid = -1
        ...
        function add_mapdata_listener(){
                    ...
                    //first time, only run 1 time
                    set_map_style(_default_strokeColor,_default_strokeWeight, _default_fillColor , _default_pointRadius)


                    //first time, only run 1 time
                    current_object_id_field_name = determine_object_id_field_name(_featurelayerJSON)
                    ...
                    // click listener
                      map.data.addListener('click', function(event) {
                        ...
                        // find user clicked object id, add it to selected pool
                        event.feature.toGeoJson(function(_geojson_clicked){ 

                            console.log('click geojson ', _geojson_clicked)
                            var ___properties = _geojson_clicked.properties
                            current_selected_objectid = ___properties[current_object_id_field_name]; // default is 'OBJECTID'

                            var index = all_selected_objectid_array.indexOf(current_selected_objectid);
                            ...
                            if (index > -1) {
                                    ...
                                    // new object id selected, add to array pool
                                    all_selected_objectid_array.push(current_selected_objectid)
                            }
                            
                            console.log('current_selected_objectid', current_selected_objectid)
                            console.log('all_selected_objectid_array', all_selected_objectid_array) 
                        });
