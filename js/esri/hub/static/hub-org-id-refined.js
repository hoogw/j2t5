...
// step 1:  http://localhost:10/json2tree/esri/hub/static/hub-org-id.html?file=hub_v1_created_before_2025_3_26.zip

// step 2:  click start button

// step 3: when finished, click download csv, json,  

// step 4:  json.zip for hub-org-id-refined.html?file=json.zip

// step 5:  use refined, http://localhost:10/json2tree/esri/hub/static/hub-org-id-refined.html?file=hub_refined_2024.zip&filter_by=
...
/**/
  ...
  async function rendering_json_to_html(json) {
    ...
    if (_results.length > 0) {
      ...
      for (var i = 0; i < _results.length; ++i){  
        ...
        //       - -  -  -  refined   - -  -  -  
        var AGOL_portalHome_url = _results[i].esri_hosted_portal_AGOL_home_page;   // true site url 
        var AGOL_portalGallery_url = AGOL_portalHome_url + portal_gallery_html

        var hub_orgId = _results[i].organization_id;
        // - - hub only - -
        var _source = _results[i].source;
        ...
        // ******************** end *************** only      calculate for hub.arcgis.com , opendata.arcgis.com    only   ****************************
          ...
          html += '<span class="context" style="font-size:17.7px;font-weight:900">'  + _source + '</span>'
          html += '&nbsp;&nbsp;&nbsp;';

          html += '<sup style="font-size:12.5px;"> AGOL-organization-home-esri-hosted-portal </sup>'
          html += '<a target="_blank"  style="font-size:16.5px;" href="' + AGOL_portalGallery_url + '">';
          html +=   AGOL_portalGallery_url 
          html += '</a>';
          html += '&nbsp;&nbsp;&nbsp;';
          ...
          if (_results[i].esri_hosted_arcgis_server_home_folder){
               ...
               var _link_url_parameter = _results[i].esri_hosted_arcgis_server_home_folder;
               if (_link_url_parameter.indexOf('http://') > -1){
                   // if resource url is http, force link protocol as http: 
                   _link_protocol = 'http:'
               }// if
               // ------ end fix bug,
                _open_link = _link_protocol + "//" + window.location.host +  '/json2tree/esri/server/folder2.html?url=' +  _results[i].esri_hosted_arcgis_server_home_folder + '&org=' + encodeURIComponent(_source).replace(/[!'()*]/g, escape);
                

                html += '<button class="click_change_style" onclick="open_link(\'' + _open_link + '\')">';
                html += '<sup style="font-size:11.5px;">AGOL-Arcgis-Server-Esri-Hosted</sup>'
                html += '<sub style="font-size:11.5px;">Show-All-Layers</sub>'
                html += '</button>';

                html += '<a target="_blank"  style="font-size:16.5px;" href="' + _results[i].esri_hosted_arcgis_server_home_folder + '">';
                html +=   _results[i].esri_hosted_arcgis_server_home_folder 
                html += '</a>';
                html += '&nbsp;&nbsp;&nbsp;';
          }
          ...
          if (_possible_link_array.length){
            ...
            for (let k = 0; k < _possible_link_array.length; k++) {
              html += '&nbsp;&nbsp;';
                  ...
                  // ------ end fix bug,
                  _open_link = _link_protocol + "//" + window.location.host +  '/json2tree/esri/server/folder2.html?url=' +  encodeURIComponent(_possible_link_array[k]) + '&org=' + encodeURIComponent(_source).replace(/[!'()*]/g, escape);
          ...
          if (_possible_link_array.length){
            ...
            for (let k = 0; k < _possible_link_array.length; k++) {
              html += '&nbsp;&nbsp;';
              html += '<a target="_blank"  style="font-size:16.5px;  background-color:#EE82EE;" href="' + _possible_link_array[k] + selfHost_portal_gallery_html + '">';
              html +=   _possible_link_array[k] + selfHost_portal_gallery_html
              html += '</a>';
            }//for
          }//if
  ...
  function build_json_for_csv(_raw_json){
    ...
    for (let i = 0; i < _raw_json.length; i++) {
          ...
          // - - hub only - -
          single_hub_item['source'] =  _raw_json[i].source;
          ...
          single_hub_item['esri_hosted_serviceXnumber'] =  _raw_json[i].esri_hosted_serviceXnumber
          single_hub_item['esri_hosted_arcgis_server_home_folder'] =  _raw_json[i].esri_hosted_arcgis_server_home_folder
          ...
          single_hub_item['esri_hosted_portal_AGOL_home_page'] =  _raw_json[i].esri_hosted_portal_AGOL_home_page;
