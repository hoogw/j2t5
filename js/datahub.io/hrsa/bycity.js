...
require([
  ...
  "esri/Graphic",
          ...
          // for city only, buffer size and relationship is special for city
          ...
          var city_layer_url = ___url_string + '/11' 


          // level 1 is state name
          var level1_field_name = 'COUNTY_NM'
          var level1_url = city_layer_url + '/query?outFields='
          ...
          // level 2 is city
          var level2_field_name = 'MCD_NM'
          var city_fips_cd_field_name = 'MCD_FIPS_CD'
          var city_fips_cd = {}
          var level2_url = city_layer_url + '/query?outFields=' + city_fips_cd_field_name + ','
    ...
    var mapimagerlayer_sub_layer // filter map image layer, use layer definition expression
          ...
          // ----- end  -----  image or tile -----  ----- 
          /**/
          var cityMapImageLayer
          var cityFeatureLayer
...
/**/
  ...
  /* .... ... ... external function ... ... ... */     
              ...
              async function work_on_value(workonthiskeyword, index){
                ...
                cityMapImageLayer = new MapImageLayer({
                                                      ...
                                                      sublayers: [{
                                                          ...
                                                          // both works
                                                          // warning: because multiple city could have same name, 'orange city', must use 'sql' 'AND' to restrict state name
                                                          ...
                                                          //definitionExpression: city_fips_cd_field_name +'=\'' + city_fips_cd[workonthiskeyword] + '\'' ,
                ...
                */
                ...
                //Zoom to city
                cityFeatureLayer = new FeatureLayer({
                    url: city_layer_url
                });
                ...
                const cityQuery = {
                                      ...
                                      // fips id works, do not have to use, in this case, not use 
                                      //where: city_fips_cd_field_name + '=\'' + city_fips_cd[workonthiskeyword] + '\'',
                ...
                cityFeatureLayer.queryFeatures(cityQuery)
                ...
                cityFeatureLayer.queryExtent(cityQuery).then(function (results) {
              ...
              // only for city layer (popup template) 
              function displayResults(results) {
                ...
                view.goTo(results.features);
                      ...
                      const popupTemplate = {
                        title: "{MCD_NM} ",
                        content: "{COUNTY_NM} &nbsp; {STATE_NM} <br> FIPS: {MCD_FIPS_CD}"
                      };
              ...
              function testRelationship(results, buffer_distance, buffer_unit){
                ...
                /*
                   why need buffer? 
                   because some case, city boundary accidently touch-overlap with rural health boundary a few meters, 
                ...
                */
                ...
                // buffer could be failed, for example, colorado -> broomfield city -> polygon have complex multipart 
                ...
                /* possible relationship 
                              ...
                              city layer :  use "within" better than 'intersects'
              ...
              async function build_level2_list(filterbykeyword, index){
                ...
                update_url_parameter('select_level2value', -1);
                    ...
                    for (let j = 0; j < raw_distinct_level2_value.features.length; j++) {
                      ...
                      city_fips_cd[_level_2_value] = raw_distinct_level2_value.features[j]['attributes'][city_fips_cd_field_name]
                    }
                    ...
                    if (level2_value_array) {
                      if ( level2_value_array.length > 0 ) {
                              ...
                              value_html += '<h2 style="display:inline">City</h2>'
                              value_html += '</br>'                            
