 ...
 /**/
                ...
                function get_map_bound(){
                  ...
                  //   ********* end ********* envelope(arcgis rest api only) cross 180th Meridian  ************ 
                          ...
                          /**/
                          // -- -- --  esri return centroid google labeling -- -- -- 

                          // only polygon can do centroid
                          if ((_url.includes('/FeatureServer')) && (featurelayer_geometrytype.includes('Polygon') || featurelayer_geometrytype.includes('polygon'))){
                             // point, line, will cause error if add this parameter
                              _url_returngeojson += '&returnCentroid=true' 
                          }//if 
                          // -- -- --  end -- -- --  esri return centroid google labeling -- -- -- 
                          /**/
...
var default_labelMarker
var highlighted_labelMarker

var labelMarker_objectIdAsKey = {} //objectid as key
var label_centroid_lat
var label_centroid_lng
var label_feature_geometry
...
var last_click_objectid

         
        function set_map_style(____labelAsFieldName, ____strokeColor, ____strokeWeight, ____fillColor , ____circle_radius){

            console.log('... ... ... set map style ... ... ... label as ... ... ... ', ____labelAsFieldName )

            remove_all_label_marker()
          

          // set style function will be called every time, you add new geojson data, or override change style, 
          // not just call 1 time, instead, it called every time, when hover, click, mouse-out because of style change
          // therefor create multiple label marker, so you have to remove duplicated label marker 
          map.data.setStyle(function(feature) {

                 // console.log(' map data setStyle feature ',feature)



                  if (____labelAsFieldName == 'nolabel'){

                    console.log('... ... ... no label applied ... ... ... ' )

                  } else {
                  
                    
                    _google_label_text = feature.getProperty(____labelAsFieldName)
                    _uniqueID = feature.getProperty(objectid_field_name)

                 
                    if (labelMarker_objectIdAsKey.hasOwnProperty(_uniqueID)){
                        // already have this label by object id
                    } else {
                  
                            // not have this label(object id), need add new label 


                           
                            // set centroid lat, long, for all situation, polygon, line, point, feature-server map-server, whatever
                            label_centroid_lat  = feature.getProperty('esri_centroid_lat')
                            label_centroid_lng  = feature.getProperty('esri_centroid_lng')

                            /*
                            // works, but not use, just keep here
                            if (featurelayer_geometrytype.includes('Point') || featurelayer_geometrytype.includes('point')){
                                // for point only, set centroid lat, long
                                label_feature_geometry = feature.getGeometry();
                                console.log('... ... ... point  geometry ... ... ... ' , label_feature_geometry) 
                                label_centroid_lat = label_feature_geometry.elements.lat()
                                label_centroid_lng  = label_feature_geometry.elements.lng()
                            }  
                            // - end - set centroid lat, long
                            */


                           
                          
                            if (_google_label_text){


                                  //console.log('... ... ... _google_label_text ... ... ... ' , _google_label_text)

                                  /*
                                      sometime, value is object, not a string, for example below
                                                      information_website: {url: "http://www.laocb.org"}
                                      I must extract the 1st value from this object
                                  */
                                  if (typeof _google_label_text === 'object' && _google_label_text !== null){ 
                                    // How to access the first property of a Javascript object https://stackoverflow.com/questions/983267/how-to-access-the-first-property-of-a-javascript-object
                                    _google_label_text = _google_label_text[Object.keys(_google_label_text)[0]]; //returns 'someVal'
                                  }




                                  // create advanced marker as label
                                  label_html = document.createElement("div");
                                  label_html.textContent = _google_label_text;
                                  label_html.className = 'black_label';    
                                  //label_html.className = 'white_label';  

                                  default_labelMarker = new google.maps.marker.AdvancedMarkerElement({
                                    map,
                                    position: { lat: label_centroid_lat, lng: label_centroid_lng }, // { lat: 37.42, lng: -122.1 },
                                    content: label_html,

                                   // no use, title, because title will popup text if mouse over marker label 
                                   // title require string only, number will cause error.
                                   // title:_google_label_text.toString(),
                                  });
                             
                                  console.log('...create a new label marker ... hover,mouseout will setstyle again then override, revert ' , _uniqueID, _google_label_text)
                                  labelMarker_objectIdAsKey[_uniqueID] = default_labelMarker
                                                             

                            } else {

                              console.log('... ... ... this label is empty ... ... ... ' )
                  ...
                  // no matter with label or not, alway return same style
                  return {
                                ...
                                icon        : {
                                  ...
                                  scale: google_marker_icon_scale_by_zoom_level[_center_zoom],
                                ...
                                strokeColor: ____strokeColor,
                                strokeWeight: ____strokeWeight
                  };// return // no matter with label or not, alway return same style
        ...
        function remove_all_label_marker(){

          if (highlighted_labelMarker){
            highlighted_labelMarker.map = null;
          }
          

          for (const key in labelMarker_objectIdAsKey) {
            if (labelMarker_objectIdAsKey.hasOwnProperty(key)) {

              // remove advanced marker https://stackoverflow.com/questions/74486201/remove-google-maps-marker-advancedmarkerview-from-map
              labelMarker_objectIdAsKey[key].map = null;
              
            }//if
          }//for

          labelMarker_objectIdAsKey = {}
          ...
          map.data.addListener('click', function(event) {
             ...
             if (click_or_hover == 'click'){
                  ...
                  show_info_outline_Tab(event)
                        ...
                        // highlight label marker by change style
                        _uniqueID = event.feature.getProperty(objectid_field_name)
                        console.log(' hover , object id is',  _uniqueID)
                        _google_label_text = event.feature.getProperty(current_labelAs)
                        


                        // remove last highlighted label marker
                        if (highlighted_labelMarker){
                              highlighted_labelMarker.map = null
                        }
                        // restore last click label marker
                        if (labelMarker_objectIdAsKey[last_click_objectid]){
                              labelMarker_objectIdAsKey[last_click_objectid].map = map
                        }
                        // hide this label
                        if (labelMarker_objectIdAsKey[_uniqueID]){
                              labelMarker_objectIdAsKey[_uniqueID].map = null
                        }
                        last_click_objectid = _uniqueID




                        // change color scale not work
                        // default scale is 1, https://developers.google.com/maps/documentation/javascript/reference/advanced-markers#PinElementOptions.scale
                        //labelMarker_objectIdAsKey[_uniqueID].content.scale = 5
                        //labelMarker_objectIdAsKey[_uniqueID].content.background = 'red'



                        // add new highlighted label marker
                        
                         
                        // set centroid lat, long, for all situation, polygon, line, point, feature-server map-server, whatever
                        label_centroid_lat  = event.feature.getProperty('esri_centroid_lat')
                        label_centroid_lng  = event.feature.getProperty('esri_centroid_lng')

                        /*
                        // works, but not use, just keep here
                        if (featurelayer_geometrytype.includes('Point') || featurelayer_geometrytype.includes('point')){
                            // for point only, set centroid lat, long
                            label_feature_geometry = event.feature.getGeometry();
                            console.log('... ... ... point  geometry ... ... ... ' , label_feature_geometry) 
                            label_centroid_lat = label_feature_geometry.elements.lat()
                            label_centroid_lng  = label_feature_geometry.elements.lng()
                        }  
                        ...
                        */
                          ...
                          if (_google_label_text){
                            ...
                            */
                            ...
                            label_html = document.createElement("div");
                            label_html.textContent = _google_label_text;
                            label_html.className = 'blue_label'; 

                           highlighted_labelMarker = new google.maps.marker.AdvancedMarkerElement({
                              map,
                              position: { lat: label_centroid_lat, lng: label_centroid_lng }, // { lat: 37.42, lng: -122.1 },
                              content: label_html,
                            ...
                            console.log('... add a new high light label marker ... hover,mouseout will setstyle again then override, revert ' , _uniqueID, _google_label_text)
          ...
          // mouse over listener
            ...
            //  --- click or hover   ---
                  if (click_or_hover == 'hover'){
                        ...
                        map.data.overrideStyle(event.feature, {
                          ...
                          // warning:
                          // override style is on top of set-style https://developers.google.com/maps/documentation/javascript/reference/data#Data
                          // that means, before override, it always first apply set-style function ( which will create a new label marker on top of old label)
                        ...
                        //  - -  clear last time clicked feature, then change from click to hover   - - 
                        // remove last highlighted label marker
                        if (highlighted_labelMarker){
                          highlighted_labelMarker.map = null
                        }
                        ...
                        // hide normal label before add new highlight label
                        if (labelMarker_objectIdAsKey[_uniqueID]){
                          labelMarker_objectIdAsKey[_uniqueID].map = null
                          console.log('hide normal label before add new highlight label')
                        }
            ...
            map.data.addListener('mouseout', function (event) {
                ...
                if (click_or_hover == 'hover'){
                  ...
                  // warning:
                  // revert style is actually implement again set-style https://developers.google.com/maps/documentation/javascript/reference/data#Data
                  // that means,  it always apply again set-style function ( which will create a new label marker on top of old label)
                       ...
                       empty_info_outline_Tab()
                        ...
                        // revert label style back to normal default
                        _uniqueID = event.feature.getProperty(objectid_field_name)
                        console.log(' mouse out , object id is',  _uniqueID)
                        

                        // remove highlighted label marker
                        if (highlighted_labelMarker){
                          highlighted_labelMarker.map = null
                        }

                        // re-show normal label marker
                        if (labelMarker_objectIdAsKey[_uniqueID]){
                          labelMarker_objectIdAsKey[_uniqueID].map = map
                          console.log('re-show normal label marker')
                        }
