...
/**/
      ...
      // warning:  share for all . js file has another set of code to get google route by click twice on map, here is different set of google route.
      function init_google_rout(){
        ...
        // --- --- get route button   --- --- 
        $("#google_route_button").on('click', show_google_route)
        $("#clear_route_button").on('click', clear_google_route)
        ...
        // --- --- travel mode   --- --- 
              ...
              $("input[type=radio][name=travel_mode_radio][value=" + travel_mode + "]").prop('checked', true);


              // event
              $('input[type=radio][name=travel_mode_radio]').on('change', function() {

                travel_mode = $("input[type='radio'][name='travel_mode_radio']:checked").val();
                console.log(" travel_mode : --  ", travel_mode);
                update_url_parameter('travel_mode', travel_mode)
        ...
        //  --- --- first time set from to value  --- ---
            param_from  = urlParams.get('from'); 
            console.log('param from',  param_from) 
            if (param_from){
              from_input_text = param_from
            }
            console.log('init from_input_text',  from_input_text) 
            $("#google-route-from").val(from_input_text)

            param_to  = urlParams.get('to'); 
            console.log('param to',  param_to) 
            if (param_to){
              to_input_text = param_to
            }
            console.log('init to_input_text',  to_input_text) 
            $("#google-route-to").val(to_input_text)
        //  --- end ---  first time set from to value   --- ---




       /**/
        // --- --- google search place --- from to box   --- --- 

              // Create the search box and link it to the UI element.
              var  google_route_from = document.getElementById('google-route-from');
              var  google_route_from_box = new google.maps.places.SearchBox(google_route_from);

               // Bias the SearchBox results towards current map's viewport.
               map.addListener('bounds_changed', function() {
                google_route_from_box.setBounds(map.getBounds());
              });
              
              // Listen for the event fired when the user selects a prediction and retrieve
              // more details for that place.
              google_route_from_box.addListener('places_changed', function() {

                from_input_text = $("#google-route-from").val()
                update_url_parameter("from", from_input_text)
                console.log('from :', from_input_text)
              });



              // Create the search box and link it to the UI element.
              var  google_route_to = document.getElementById('google-route-to');
              var  google_route_to_box = new google.maps.places.SearchBox(google_route_to);

              // Bias the SearchBox results towards current map's viewport.
              map.addListener('bounds_changed', function() {
                google_route_to_box.setBounds(map.getBounds());
              });
              
              // Listen for the event fired when the user selects a prediction and retrieve
              // more details for that place.
              google_route_to_box.addListener('places_changed', function() {

                to_input_text = $("#google-route-to").val()
                update_url_parameter("to", to_input_text)
                console.log('to :', to_input_text)
              });

        //  --- end --- --- --- google search place --- from to box   --- --- 
        /**/
        ...
        // first time, only run 1 time, to show route
        show_google_route()
      ...
      function show_google_route() {
        ...
        var start = $("#google-route-from").val() //route_start_location;
        var end = $("#google-route-to").val() //route_end_location;
        ...
        if ((start) && (end)){
          ...
          try {
            ...
            if (typeof (JSON.parse(start)) === 'object' && (JSON.parse(start)) !== null){
              start = JSON.parse(start)
              console.log(' start is lat lng point object ',start)
            }//if 

          } catch {

            console.log(' start is address string ', start)
          }// catuch
                      ...
                      directionsService
                        .route({
                          origin: start,
                          destination: end,
                          travelMode: google.maps.TravelMode[travel_mode],  // google.maps.TravelMode.DRIVING,
                        })
                        .then((response) => {
...
/**/
      ...
      function poi_viewer_for_route(___properties){
        ...
        $("#google-route-to").val(___properties.location.join(","))
