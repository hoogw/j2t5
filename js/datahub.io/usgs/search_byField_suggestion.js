...
/**/
 ...
 // -------- ---------- autocomplete --------  ------ // no suggestion for all field search

                //  https://www.w3schools.com/howto/tryit.asp?filename=tryhow_js_autocomplete

                var possible_value_array = []

                var max_option_item_count = 999
                var current_option_item = 0
                ...
                function autocomplete(inp, arr) {
                  ...
                  inp.addEventListener("input", function(e) {
                    ...
                    // no suggestion for all field search
                    if (current_searchByField == 'allfield'){
                      console.log(' all field search, would not provide suggestion, skip suggestion')
                    } else {
                              // no suggestion for all field search
                      ...
                      $('#possible_btn_span').text('')

                      console.log('auto complete input value:', this.value)
                      ...
                      for (i = 0; i < arr.length; i++) {
                        ...
                        /* not use, check if the item starts with the same letters as the text field value:*/
                        //if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                        // in use,  
                        var indexOfFirst = arr[i].toUpperCase().indexOf(val.toUpperCase());
                        if (indexOfFirst > -1) {
                           
                          //console.log(' includes , user type ', val,  arr[i])

                          current_option_item += 1
                          $('#possible_btn_span').text(current_option_item)
                          if (current_option_item > max_option_item_count){
                            $('#possible_btn_span').text(current_option_item + '+')
                            break; // for loop
                          }
                         ...
                         /*
                          ...
                          console.log(' pref- ', arr[i].substr(0, indexOfFirst))
                          console.log(' self- ', arr[i].substr(indexOfFirst, val.length))
                          console.log(' surf- ', arr[i].substr(indexOfFirst + val.length))
                          */

                          b.innerHTML = arr[i].substr(0, indexOfFirst);
                          b.innerHTML += "<strong>" + arr[i].substr(indexOfFirst, val.length) + "</strong>";
                          b.innerHTML += arr[i].substr(indexOfFirst + val.length);
                          ...
                          b.addEventListener("click", function(e) {
                              ...
                              console.log('auto complete * * * * click event * * * * need to auto search layer now * * * * ')
                              $('#possible_btn_span').text('')
                              search_layer_now()
                  ...
                  inp.addEventListener("keydown", function(e) {
                    ...
                    if (current_searchByField == 'allfield'){
                      ...
                      // no suggestion for all field search
                                  ...
                                  console.log('auto complete keydown, keycode:', e.keyCode)
                  ...
                  function addActive(x) {
                    ...
                    console.log('auto complete addActive:', x)
                  ...
                  function removeActive(x) {
                    ...
                    console.log('auto complete removeActive:', x)
                  ...
                  function closeAllLists(elmnt) {
                    ...
                    console.log('auto complete closeAllLists:',    elmnt)
                    ...
                    for (var i = 0; i < x.length; i++) {
                      if (elmnt != x[i] && elmnt != inp) {
                        ...
                        console.log('auto complete closeAllLists - - - except one passed as argument - - -',    x[i])
  ...
  // -------- end ---------- autocomplete -------------- // no suggestion for all field search



        var _distinct_or_groupby = 'groupby'    // default,    "groupby" or "distinct"

        async function get_possible_value(_fieldName, filterValueList_by_keyword){
        ...
        console.log(' supports Statistics,  stringified out Statistics,   encoded out Statistics ==== ', _supportsStatistics, stringified_outStatistics, encoded_outStatistics)
                          ...
                          // - - - filter value list  - - - 
                          /**/
                                  var where_clause_for_filterValueList 
                                  var encoded_where_clause_for_filterValueList
                                  var _url_distinctFieldValue_groupby

                                  if (filterValueList_by_keyword){
                                        // filtered value list
                                        if (____fieldType_lowerCase.includes('string')){ 
                                              
                                              //string, both esri, socrata, SQL are case-sensitive, must use LOWER ( field name ) = keyword.toLowerCase() 
                                              //where_clause_for_filterValueList = _fieldName + " like '%" + filterValueList_by_keyword + "%' "
                                              where_clause_for_filterValueList = "LOWER(" + _fieldName + ") like '%" + filterValueList_by_keyword + "%' "
                                                  ...
                                                  // only for number, integer, or double 
                                                    where_clause_for_filterValueList = "CAST(" + _fieldName + " as varchar(255)) like '%" + filterValueList_by_keyword + "%' "
                                        }


                                        encoded_where_clause_for_filterValueList = encodeURIComponent(where_clause_for_filterValueList)
                                        //_url_distinctFieldValue_groupby =  _url + '/'+  _layer_id + '/query?'+  'where=' + encoded_where_clause_for_filterValueList + '&outStatistics=' + encoded_outStatistics + '&groupByFieldsForStatistics=' + _fieldName  +'&f=pjson'
                                        _url_distinctFieldValue_groupby =  background_layer_url + '/query?'+  'where=' + encoded_where_clause_for_filterValueList + '&outStatistics=' + encoded_outStatistics + '&groupByFieldsForStatistics=' + _fieldName  +'&f=pjson'
                                        
                                        console.log( 'filtered value list, url, group by field/count, %%%%%%  group by  %%%%%% ', filterValueList_by_keyword,   _url_distinctFieldValue_groupby)
                                      } else {
                                        // all value list
                                        //_url_distinctFieldValue_groupby =  _url + '/'+  _layer_id + '/query?outStatistics=' + encoded_outStatistics + '&groupByFieldsForStatistics=' + _fieldName  +'&f=pjson'
                                        _url_distinctFieldValue_groupby =  background_layer_url + '/query?outStatistics=' + encoded_outStatistics + '&groupByFieldsForStatistics=' + _fieldName  +'&f=pjson'
                                        
                                        console.log( 'all value list, url, group by field/count, %%%%%%  group by  %%%%%% ',  _url_distinctFieldValue_groupby)
                                      }
                          ...
                          /**/
                                                                      ...
                                                                      */
                                                                            ...
                                                                            var _url_esri_groupby_null_fix =  background_layer_url + '/query?' + 'where=' + _where_is_null + '&returnGeometry=false&returnCountOnly=true&outSR=4326&f=pjson'
            ...
            //  ======  =====  end =======    group by  ======  ===== ======= 
                      ...
                      // ======  ===== =======    distinct  ======  ===== =======  
                                  ...
                                  /**/
                                                ...
                                                if (filterValueList_by_keyword){
                                                      ...
                                                      encoded_where_clause_for_filterValueList = encodeURIComponent(where_clause_for_filterValueList)
                                                      //_url_distinctFieldValue_distinct =  _url + '/'+  _layer_id + '/query?' + 'where=' + encoded_where_clause_for_filterValueList + '&outFields=' + _fieldName + '&f=pjson&returnGeometry=false&returnCountOnly=false&returnDistinctValues=true'
                                                      _url_distinctFieldValue_distinct =  background_layer_url + '/query?' + 'where=' + encoded_where_clause_for_filterValueList + '&outFields=' + _fieldName + '&f=pjson&returnGeometry=false&returnCountOnly=false&returnDistinctValues=true'
                                  
                                                    } else {

                                                      // all value list
                                                      //_url_distinctFieldValue_distinct =  _url + '/'+  _layer_id + '/query?outFields=' + _fieldName + '&f=pjson&returnGeometry=false&returnCountOnly=false&returnDistinctValues=true&where=1=1'
                                                      _url_distinctFieldValue_distinct =  background_layer_url + '/query?outFields=' + _fieldName + '&f=pjson&returnGeometry=false&returnCountOnly=false&returnDistinctValues=true&where=1=1'
                  ...
                  if (real_distinct_fieldvalue_array) {
                    if ( real_distinct_fieldvalue_array.length > 0 ) {
                          for (var r = 0; r < real_distinct_fieldvalue_array.length; r++) { 
                            ...
                            // must be array of string, if number, must convert to string
                            if (isNaN(_this_field_value)){
                                  possible_value_array.push(_this_field_value)
                            } else {
                                  // must be array of string, if number, must convert to string
                                  possible_value_array.push(_this_field_value.toString())
                            }
                  ...
                  console.log(' return,   possible unique value array :  ',  possible_value_array)
                  /*initiate the autocomplete function on the "myInput" element, and pass along the possible value array array as possible autocomplete values:*/
                  autocomplete(document.getElementById("search_text_esri"), possible_value_array);
        }



        // no suggestion for all field search
        function init_search_suggestion(){
          ...
          $( "#search_button_fuzzy" ).on( "click", function(){
            search_layer_now()
          });
    
          $( "#search_text_esri" ).on( "search", function(){
            search_layer_now()
          }); 
          ...
          $( "#search_text_esri" ).on( "keyup", function(){
            ...
            //$('#result_div').html('')
            searchText = $('#search_text_esri').val().trim().toLowerCase();
            console.log('user just type , do not search yet, only update param', searchText)
            update_url_parameter('search', searchText);
            ...
            if (current_searchByField == 'allfield'){
              ...
              // no suggestion for all field search
              get_possible_value(current_searchByField, searchText)
            }
