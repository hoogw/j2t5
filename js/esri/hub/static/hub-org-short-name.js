...
// test sample:  http://localhost:10/json2tree/esri/hub/static/opendata-org-short-name.html?file=opendata_v3_region_us_9972.zip&filter_by=





// step 1: (8026)  http://localhost:10/json2tree/esri/hub/static/hub-org-short-name.html?file=hub_v1_created_before_2025_3_26.zip

// step 2:  click start button

// step 3: when finished, click download csv, json,  

// step 4:  json.zip for hub-org-short-name-refined.html?file=json.zip

// step 5:  use refined, http://localhost:10/json2tree/esri/hub/static/hub-org-short-name-refined.html?file=hub_refined_2024.zip&filter_by=
    ...
    if (_results.length > 0) {
      ...
      for (var i = 0; i < _results.length; ++i){  
        ...
        // ******************** end *************** only      calculate for hub.arcgis.com , opendata.arcgis.com    only   ****************************
          ...
          html += '<span class="context" style="font-size:21.7px;font-weight:300">'  + _source + '</span>'
          html += '&nbsp;&nbsp;&nbsp;';
          ...
          html += '<span class="context" style="font-size:15.5px;font-weight:100;">'  + hubsite_short_name + '</span>'
          html += '&nbsp;&nbsp;&nbsp;';
          ...
          //html += '<span class="context" style="font-size:10.5px;font-weight:100;">'  + ___siteUrl + '</span>'
          //html += '&nbsp;&nbsp;&nbsp;';

          //title = hub site title,  
          //html += '<span class="context" style="font-size:12.2px;font-weight:600;">'  + _title + '</span>' 
          //html += '&nbsp;&nbsp;&nbsp;';
          //snippet = hub site title's snippet, 
          //html += '<span class="context" style="font-size:10.2px;font-weight:100">'  + hub_snippet + '</span>'
          //html += '&nbsp;&nbsp;&nbsp;';
...
// every day 100 free, 
// manually load 0-5, 6-10, ...
//if ((i == 1) || (i == 1) || (i == 1) || (i == 1) || (i == 1)){  
//  if ((i > 40) && (i < 50)){
                    
      //  .. ..  .. ajax streaming  .. ..  .. 
      if (run_ajax_status){

        if (ajaxedResult_orgshortname_askey.hasOwnProperty(org_short_name)){

              console.log('skip index already done ', i, )

        } else {
        
          console.log('this org short name not in list, need ajax, index is ', i+1, )
          var orgshortname = {
            selfHosted_rest: [],
            selfHosted_portal: [],
          }
                    ...
                    // do not use 'arcgis' it will bring other arcgis.com link, use rest services instead, use folder layer to improve the accuracy
                    // use both short-name and org-name to improve accuracy
                    google_api_url = google_api_url_template + '&q=' + org_short_name + ' ' + _source + ' rest services folder layer'
                    //google_api_url = google_api_url_template + '&q=' + org_short_name + ' ' + _source + " '/rest/services' folder layer"

                    // test only

                     // not work
                      //google_api_url = google_api_url_template + '&q=' + 'titusville' + 'rest services folder'

                      // 64
                     // google_api_url = google_api_url_template + '&q=' + 'hartfordgis City of Hartford ' + ' rest services folder layer portal'

                      // 80
                      //google_api_url = google_api_url_template + '&q=' + 'morganton' + ' rest services folder layer portal home'
                      //google_api_url = google_api_url_template + '&q=' + 'morganton City of Morganton GIS ' + ' rest services folder layer portal home'

                      // 48
                     // google_api_url = google_api_url_template + '&q=' + 'lakecountyil Lake County, Illinois ' + ' rest services folder layer portal home'
                      // not working, must use folder layer
                     // google_api_url = google_api_url_template + '&q=' + 'lakecountyil Lake County, Illinois ' + ' rest services'

                      // 83 (fail but human search ok)
                      //google_api_url = google_api_url_template + '&q=' + 'Topeka City of Topeka ' + ' rest services folder layer '

                      // add 'folder' works
                      //google_api_url = google_api_url_template + '&q=' + 'lakecountyil' + ' arcgis rest "Folder: /"'
                      //google_api_url = google_api_url_template + '&q=' + 'lakecountyil' + ' arcgis rest services folder'
                    ...
                    message_html = '<span style="font-size:30px;  background-color:#FFA500;">' + (i+1) + '</span> - <span>' + _source + '</span> - <span>' + google_api_url
                    $("#message-span").html(message_html)

                    var response = await ajax_getjson_common(google_api_url, best_timeout);
                    
                    console.log('google search api ', i+1, response);
                    ...
                    if ((response) && (response.items)){
                         ...
                         var google_items = response.items
                         var google_link = []
                         var _rest_service_root
                         var _selfHosted_domain
                         var _portal_json_url
                         var _portal_home_url
                      
                         for (let g = 0; g < google_items.length; g++) {
                          google_link = google_items[g].link

                          if (google_link.includes('/rest/services/')){

                            _selfHosted_domain = getDomain(google_link)
                            console.log('_selfHosted_domain', _selfHosted_domain)
                            
                            _rest_service_root = getSubstringBefore(google_link, '/rest/services/')
                            console.log('_rest_service_root', _rest_service_root)




                            if (possible_selfHosted_rest_array.includes(_rest_service_root)){
                                   // duplicate, nothing to do
                            } else {


                              message_html = '<span style="font-size:30px;  background-color:#FFA500;">' + (i+1) + '</span> - <span>' + _source + '</span> - <span>' + _rest_service_root
                              $("#message-span").html(message_html)

                                        possible_selfHosted_rest_array.push(_rest_service_root)

                                        _portal_json_url = 'https://' +  _selfHosted_domain + '/portal/sharing/rest/portals/self?f=json'
                                        _portal_home_url = 'https://' +  _selfHosted_domain + '/portal/home' + '/gallery.html'

                                        try{

                                          var portal_json_response = await ajax_getjson_common(_portal_json_url, best_timeout);
                                          if (portal_json_response){
                                            if (portal_json_response.currentVersion){
                                              console.log(' portal found found found found ', i+1, response.currentVersion, _portal_json_url)
                                              possible_selfHosted_portal_array.push(_portal_home_url)
                                            }//if
                                          }//if 

                                        } catch {

                                           console.log(' portal test failed ', i+1, response.currentVersion, _portal_json_url )
                                        }
                        ...
                        orgshortname.selfHosted_rest   = possible_selfHosted_rest_array
                        orgshortname.selfHosted_portal = possible_selfHosted_portal_array
                        ajaxedResult_orgshortname_askey[org_short_name] = orgshortname
...
//  .. ..  .. show result  .. ..  .. 
var _orgshortname_obj = ajaxedResult_orgshortname_askey[org_short_name]

if (_orgshortname_obj){


  var _possible_selfHosted_rest_array = _orgshortname_obj['selfHosted_rest']
  ...
  var _possible_selfHosted_portal_array = _orgshortname_obj['selfHosted_portal']
  if (_possible_selfHosted_portal_array){
    for (let k = 0; k < _possible_selfHosted_portal_array.length; k++) {
      html += '&nbsp;&nbsp;';
      html += '<a target="_blank"  style="font-size:18.8px; font-weight:800; background-color:#FFD700;" href="' + _possible_selfHosted_portal_array[k] + '">';
  ...
  function build_json_for_csv(_raw_json){
    ...
    for (let i = 0; i < _raw_json.length; i++) {
      ...
      if (hub_orgId){ esri_hosted_arcgis_server_url = 'https://services.arcgis.com/'  + hub_orgId + '/arcgis/rest/services'}
          ...
          // have google custom search api results
          if (ajaxedResult_orgshortname_askey.hasOwnProperty(org_short_name)){
                console.log(' download ', org_short_name,  ajaxedResult_orgshortname_askey[org_short_name])
                var orgshortname_item = ajaxedResult_orgshortname_askey[org_short_name]

                single_hub_item['self_hosted_arcgis_server_home_folder'] =  orgshortname_item.selfHosted_rest
                single_hub_item['self_hosted_enterprise_portal_home_page'] =  orgshortname_item.selfHosted_portal
          }//if have google custom search api results
          
          single_hub_item['hub_site_url'] =  ___siteUrl  
...
//  - - -   org-short-name    - - - 
var unique_orgshortname_array = []

      // { org-short-name:{ 
      //     selfHosted_rest     : {}
      //     selfHosted_portal  : [ url1, url2, url3 ...]
      //
      //          } ,
      //
      // }
var ajaxedResult_orgshortname_askey = {} // org-short-name as key
...
/**/
    ...
    function remove_duplicate_orgshortname(_raw_inputCurrent){

      var unique_input_currrent = []

      for (var i = 0; i < _raw_inputCurrent.length; ++i){ 

        var ___siteUrl = _raw_inputCurrent[i].properties.url;   // true site url 
        ...
        if (org_short_name){
                    ...
                    if (unique_orgshortname_array.includes(org_short_name)){
                      // nothing to do
                    } else {
                      unique_orgshortname_array.push(org_short_name)
                                                                            ...
                                                                            entries[0].getData(new zip.TextWriter(), function(text) {
                                                                             ...
                                                                             // console.log(text);
                                                                               ...
                                                                               //..........load into json-viewer,  unzipped text is string .........
                                                                                        ...
                                                                                        input_current = remove_duplicate_orgshortname(input_current)
