  ...
  //  - - - esri poi  - - - browse only - - - 
  
  //self-run
  (async function init_map_component_event(){ //  - - - esri poi  - - - browse only - - -
  ...
  circle_array = []
     ...
     async function init_feature_layer_view(){
        ...
        arcgisMap.addEventListener("arcgisViewClick", (event) => {
          ...
          //$("#message").html('lat:' + event.detail.mapPoint.latitude + '      lng:' +  event.detail.mapPoint.longitude)
        }); // view . on . click
...
arcgisMap.addEventListener("arcgisViewPointerMove", (event) => {
 ...
 /**/
 //  -  -  - esri guided ring  -  -  - only for pan-zoom
 /**/
      console.log('a r c g i s V i e w P o i n t e r M o v e - for - h o v e r event', event )
            var _map_center_point = arcgisMap.center             
            _center_radius_in_meter = get_center_radius_in_map_bound() 
            console.log(' esri guided ring _map_center_point', _map_center_point)
            console.log(' esri guided ring _center_radius_in_meter', _center_radius_in_meter)
         
            var guided_ring_lng = _map_center_point.longitude;
            var guided_ring_lat = _map_center_point.latitude;
            console.log(' * * * * guided_ring * * * > > > lng, lat ', guided_ring_lng, guided_ring_lat)

            
            drawing_circle_guided_ring(_center_radius_in_meter, guided_ring_lng, guided_ring_lat)
/**/
//  -  -  - end  -  -  -  esri guided ring    -  -  - 
/**/
...
function browse_poi_when_view_changed(){
  ...
  if (arcgisMap.extent){ // fix bug, view change, but view may not ready yet, 
            ...
            nearby_poi(_center_radius_in_meter, _map_center_point.longitude, _map_center_point.latitude)
  }//if
