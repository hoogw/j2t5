        ...
        var current_circle_radius
          ...
          function setLimit_onNearbyCircleRadius(){
            ...
            if (_center_radius_in_meter > max_google_poi_radius_meter){
                _center_radius_in_meter = max_google_poi_radius_meter
            }
            console.log('google nearby limit 50k _center_radius_in_meter',  _center_radius_in_meter)
              ...
              display_count_info(_layer, _current_area_count_of_feature, _total_count_of_feature, _current_rendering_feature)
                  ...
                  function add_mapdata_listener(){
                    ...
                    map.data.addListener('click', function(event) {
                      ...
                      show_info_outline_Tab_nonPOI(event)

                            /**/
                            //  --- google poi auto    --- 
                            /**/

                            console.log('you select a polygon feature ', event)
                            ...
                            if(event.feature.getGeometry().getType()==='Polygon'){
                                ...
                                console.log(' selected polygon bound', bounds)

                                var boundNorthEast = bounds.getNorthEast();
                                var boundSouthWest = bounds.getSouthWest();
                                var northLatMax = boundNorthEast.lat()
                                var southLatMin = boundSouthWest.lat()
                                var eastLngMax = boundNorthEast.lng()
                                var westLngMin = boundSouthWest.lng()

                                console.log('northLatMax, southLatMin', northLatMax, southLatMin)
                                console.log('eastLngMax, westLngMin', eastLngMax, westLngMin)


                                selected_bounds_north = northLatMax;
                                selected_bounds_east = eastLngMax;
                                selected_bounds_south = southLatMin;
                                selected_bounds_west = westLngMin;

                              } else {

                                console.log(' you select line or point, not polygon, only polygon are valid')
...
/**/
          ...
          function add_map_listener_idle(){   
            ...
            // ---  ---  marker-icon,label, size change based on zoom level  ---  ---  --- 
            var listener_zoom_changed =  map.addListener('zoom_changed', function() { 
              console.log(' . . . map zoom changed event , update all marker size and label font  . . .  ')
...
/**/
            ...
            $("#auto_start_button").on("click", function() {
               auto_draw_circle()
            });
...
/**/
    ...
    var grid_firstColumnCenterArray = []
        ...
        var selected_shape
        var selected_bounds
        var selected_bounds_north
        var selected_bounds_south
        var selected_bounds_west
        var selected_bounds_east

        function auto_draw_circle(){

            calculate_grid_center(current_circle_radius)
            draw_circle_on_grid(current_circle_radius)
            nearby_poi_auto_grid(current_circle_radius)
        }
    ...
    function calculate_grid_center(_radiusMeter){
      ...
      var NW_point = new google.maps.LatLng(selected_bounds_north, selected_bounds_west)
      console.log('NW_point', NW_point)
      
      // 0,0
      var new_grid_center_point = google.maps.geometry.spherical.computeOffset(NW_point,  _radiusMeter, 135)
      var new_grid_center_lng = new_grid_center_point.lng();
      var new_grid_center_lat = new_grid_center_point.lat();
      ...
      while (new_grid_center_lat > selected_bounds_south) {
         new_grid_center_point = google.maps.geometry.spherical.computeOffset(new_grid_center_point,  _auto_grid_side, 180,)
         new_grid_center_lng = new_grid_center_point.lng();
         new_grid_center_lat = new_grid_center_point.lat();
         grid_center_array.push(new_grid_center_point)
         grid_firstColumnCenterArray.push(new_grid_center_point)
      }// while
      ...
      for (let i = 0; i < (grid_firstColumnCenterArray.length); i++) {
        ...
        while (new_grid_center_lng < selected_bounds_east) {
         new_grid_center_point = google.maps.geometry.spherical.computeOffset(new_grid_center_point,  _auto_grid_side, 90,)
         new_grid_center_lng = new_grid_center_point.lng();
         new_grid_center_lat = new_grid_center_point.lat();
         grid_center_array.push(new_grid_center_point)
        }// while
    ...
    // * google only *
    function draw_circle_on_grid(_radiusMeter){ // * google only *
   ...
   async function nearby_poi_auto_grid(_radiusMeter){
      ...
      for (let i = 0; i < grid_center_array.length; i++) {
        ...
        _centerLng = grid_center_array[i].lng();
        _centerLat = grid_center_array[i].lat();
